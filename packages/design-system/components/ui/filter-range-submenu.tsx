...
export function RangeSubmenu({
  ...
  // Re-sync the drafts when the committed bounds change from outside (clear-all,
  // a deep-linked value, the host's normalization rewriting the bound).
  useEffect(() => {
    setMinDraft(toDraft(group.min));
  }, [group.min]);
  useEffect(() => {
    setMaxDraft(toDraft(group.max));
  }, [group.max]);
    ...
    <DropdownMenuSub>
      ...
      <DropdownMenuPortal>
        <DropdownMenuSubContent className={group.submenuClassName ?? "w-56"}>
          {/* Radix's roving-focus only targets registered menu items, and these
              native inputs are not items — so on ArrowRight-open, pull keyboard
              focus into the Min field instead of leaving both bounds click-only. */}
