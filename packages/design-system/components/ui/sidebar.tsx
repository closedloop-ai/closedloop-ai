...
import { useIsMobile } from "@closedloop-ai/design-system/hooks/use-mobile"
  ...
  return (
    <main
      ...
      className={cn(
        ...
        "md:peer-data-[variant=inset]:m-2 md:peer-data-[variant=inset]:overflow-clip md:peer-data-[variant=inset]:rounded-xl",
        className
      )}
      {...props}
    />
...
function SidebarGroupLabel({
  ...
  return (
    <Comp
      ...
      className={cn(
        "text-sidebar-foreground/70 ring-sidebar-ring flex h-8 shrink-0 items-center rounded-md pl-2 pr-1 text-xs font-medium outline-hidden transition-[margin,opacity] duration-200 ease-linear focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0",
...
function SidebarMenuSkeleton({
  ...
  // Use fixed width to avoid hydration mismatch (Math.random() differs server vs client)
  const width = "70%"
