...
import { buildDocumentChatContext } from "../document-context";
...
describe("buildDocumentChatContext", () => {
  test("includes metadata fields (type, title, slug, url)", () => {
    const ctx = buildDocumentChatContext(
      { ...DOCUMENT_BASE },
      {
        available: true,
    ...
    expect(ctx).toContain("Document type: plan");
    ...
    expect(ctx).toContain("DRAFT -> IN_PROGRESS");
  ...
  test("includes MCP instructions optimistically when availability is null (claude)", () => {
    const ctx = buildDocumentChatContext({ ...DOCUMENT_BASE }, null);
    expect(ctx).toContain("get-document");
    expect(ctx).toContain("READ-ONLY by default");
    expect(ctx).toContain("DRAFT -> IN_PROGRESS");
  });
  ...
  test("omits MCP instructions and falls back to inline content when MCP unavailable", () => {
    const ctx = buildDocumentChatContext(
      { ...DOCUMENT_BASE, inlineContent: "## Body\n\nplan body" },
      {
        available: false,
        serverName: null,
        matchedUrl: null,
    ...
    expect(ctx).not.toContain("get-document");
    expect(ctx).toContain("## Document Content");
  ...
  test("emits PRD-specific lifecycle note for prd-type documents", () => {
    const ctx = buildDocumentChatContext(
      {
        type: "prd",
        slug: "PRD-456",
    ...
    expect(ctx).toContain("Document type: prd");
