...
export type SubagentBlockProps = {
  ...
  /** Accepted for caller compatibility; subagent text is not yet highlighted. */
  searchQuery?: string;
};
        ...
        <div className="overflow-hidden">
          <div className="space-y-2 bg-orange-400/[0.02] px-3 py-2 dark:bg-orange-400/[0.03]">
            {content.map((block, idx) =>
              renderSubagentContentBlock(block, idx, expandedTools, toggleTool)
            )}
          </div>
        </div>
      </div>
...
/**
 * Render a single subagent content block by type. Extracted from the component
 * body so the per-block branching doesn't inflate the component's complexity.
 */
function renderSubagentContentBlock(
  block: ContentBlock,
  idx: number,
  ...
  if (block.type === "tool_use") {
    ...
    // CollapsibleBlock only renders children when expanded, so don't pay for
    // the JSON.stringify of the (potentially large) tool input while collapsed.
    return (
      <CollapsibleBlock
        icon={Wrench}
        id={id}
        ...
        variant="tool"
      >
        {isExpanded ? JSON.stringify(block.input, null, 2) : ""}
      </CollapsibleBlock>
    );
  }
