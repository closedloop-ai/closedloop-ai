...
function formatInlineBold(text: string): React.ReactNode {
  ...
  return parts.map((part, i) => {
    const boldMatch = BOLD_SEGMENT_REGEX.exec(part);
    if (boldMatch) {
...
// Matches a single `**bold**` segment (hoisted to module scope so it isn't
// recompiled inside the formatInlineBold map callback).
