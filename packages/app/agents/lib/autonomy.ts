 ...
 * classifier are the runtime-agnostic SSOT in
 * `@repo/api/src/session-autonomy-tiers` (FEA-2094) so every surface that
 * classifies the score — the session-detail Properties panel, the sessions-list
 * "Autonomy" column, and the Autonomy filter facet — stays in sync. This module
 * re-exports the UI-facing label helpers.
 */
import {
  type AutonomyTier,
...
export type { AutonomyTier } from "@repo/api/src/session-autonomy-tiers";
