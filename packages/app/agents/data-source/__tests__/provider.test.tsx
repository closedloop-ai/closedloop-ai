...
import type { AgentSessionsDataSource } from "../agent-sessions-data-source";
 ...
 */
function fakeSource(scope: string, total: number): AgentSessionsDataSource {
  return {
    scope,
...
describe("agent sessions data source provider", () => {
  it("scopes the cache by source so two sources under one QueryClient never cross-contaminate (FEA-1834)", async () => {
    ...
    render(
      ...
      >
        <AgentSessionsDataSourceProvider dataSource={fakeSource("local", 7)}>
