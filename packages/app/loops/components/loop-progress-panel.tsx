...
// `DisplayStatus` is defined alongside its color/label maps in the design system
// (status-badge.tsx) so the maps stay exhaustive against the union.
...
function deriveDisplayStatus(
  ...
  if (lastEvent?.type === LoopEventType.Completed) {
    return "COMPLETED";
  }
  if (lastEvent?.type === LoopEventType.Error) {
    return "FAILED";
  }
  if (lastEvent?.type === LoopEventType.Cancelled) {
    return "CANCELLED";
  }
...
function formatTimestamp(isoString: string): string {
  return formatTimeOrFallback(isoString, {
    hour12: false,
    includeSeconds: true,
  });
}
...
function useElapsedTime(startTimestamp: string | null, isRunning: boolean) {
  ...
  useEffect(() => {
    const startTime = getValidTimestampMs(startTimestamp);
  ...
  useEffect(() => {
    if (!isRunning) {
      const startTime = getValidTimestampMs(startTimestamp);
      setElapsed(startTime === null ? 0 : Date.now() - startTime);
    }
...
export function LoopProgressPanel({
  ...
  const completedEvent = events.find(
    (e): e is LoopEventCompleted => e.type === LoopEventType.Completed
  );
  const errorEvent = events.find(
    (e): e is LoopEventError => e.type === LoopEventType.Error
  );
  const tokensInput =
  ...
  // Derive start timestamp from the first event with a usable timestamp.
  // Malformed timestamps are still rendered in the timeline as fallback text,
  // but they must not poison the elapsed-time footer with NaN.
  const startTimestamp = getFirstValidEventTimestamp(events);
...
function getFirstValidEventTimestamp(events: LoopEvent[]): string | null {
  for (const event of events) {
    const timestamp = event.timestamp;
    if (timestamp && getValidTimestampMs(timestamp) !== null) {
      return timestamp;
    }
  }
  return null;
