...
vi.mock("@repo/app/loops/hooks/use-loop-polling", () => ({
  useLoopPolling: vi.fn(),
}));

vi.mock("@repo/app/loops/hooks/use-loop-stream", () => ({
  useLoopStream: vi.fn(),
}));

// --- Imports ---

import { useLoopPolling } from "@repo/app/loops/hooks/use-loop-polling";
import { useLoopStream } from "@repo/app/loops/hooks/use-loop-stream";
import {
...
const TOKEN_USAGE_REGEX = /1\.50k in \/ 800 out/;
...
const UNKNOWN_SKILL_TITLE = /Closedloop plugin command unavailable/;
const UNKNOWN_SKILL_DESCRIPTION =
  /Claude could not find the required Closedloop plugin command for this loop\./;
const RAW_RUNNER_ERROR_LABEL = /^RUNNER_ERROR$/;
const GENERIC_RUNNER_ERROR_TITLE = /^Runner failed$/;
const NAN_REGEX = /NaN/;
...
describe("ErrorEvent in LoopProgressPanel — tokenUsage", () => {
  ...
  it("renders unknown-skill runner subcode with specific plugin remediation copy", () => {
    setupMocks([RUNNER_UNKNOWN_SKILL_EVENT]);
    render(<LoopProgressPanel loopId="loop-unknown-skill" />);

    expect(screen.getAllByText(UNKNOWN_SKILL_TITLE).length).toBeGreaterThan(0);
    expect(screen.getByText(UNKNOWN_SKILL_DESCRIPTION)).toBeInTheDocument();
    expect(screen.queryByText(RAW_RUNNER_ERROR_LABEL)).not.toBeInTheDocument();
    expect(
      screen.queryByText(GENERIC_RUNNER_ERROR_TITLE)
    ).not.toBeInTheDocument();
  });
});

describe("LoopProgressPanel malformed timestamps", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("renders malformed loop event timestamps as fallback text without throwing", () => {
    setupMocks([
      {
        type: "started",
        loopId: "loop-1",
    ...
    expect(() => render(<LoopProgressPanel loopId="loop-1" />)).not.toThrow();
    expect(screen.getByText("not-a-date")).toBeInTheDocument();
    expect(screen.queryByText(NAN_REGEX)).not.toBeInTheDocument();
  });

  it("does not render NaN elapsed time when the first event timestamp is malformed", () => {
    setupMocks([
      {
        type: "started",
        loopId: "loop-1",
