...
vi.mock("@repo/app/shared/lib/date-utils", () => ({
  formatRelativeTime: () => "just now",
  formatDateTime: () => "Jan 1, 2026 00:00:00",
  formatRelativeTimeOrFallback: () => "just now",
  formatDateTimeOrFallback: () => "Jan 1, 2026 00:00:00",
}));
...
const UNKNOWN_SKILL_SUMMARY_REGEX =
  /Closedloop plugin command unavailable: Claude could not find the required Closedloop plugin command/;
...
describe("LoopAuditLog EventRow — error expandability", () => {
  ...
  it("error row renders unknown-skill runner subcode as specific plugin guidance", () => {
    ...
    expect(screen.getByText(UNKNOWN_SKILL_SUMMARY_REGEX)).toBeInTheDocument();
    expect(screen.queryByText(RAW_RUNNER_ERROR_REGEX)).not.toBeInTheDocument();
    expect(
      screen.queryByText(GENERIC_RUNNER_ERROR_SUMMARY_REGEX)
