...
describe("createResolveRoomsInfo", () => {
  ...
  describe("room ID parsing", () => {
    it("returns undefined url when server returns no metadata", async () => {
      const resolver = createResolveRoomsInfo(organizationId);
      const result = await resolver({
  ...
  describe("server-side resolution", () => {
    ...
    it("returns undefined url when server returns error", async () => {
      mockFetch.mockResolvedValueOnce({
        ok: false,
        status: 500,
      });
    ...
    it("returns undefined url when fetch throws", async () => {
  ...
  describe("room info structure", () => {
    it("returns correct RoomInfo structure with undefined url when no server metadata", async () => {
      const resolver = createResolveRoomsInfo(organizationId);
      const result = await resolver({
    ...
    it("returns undefined url when server returns no metadata for slug", async () => {
      const resolver = createResolveRoomsInfo(organizationId);
      const result = await resolver({
