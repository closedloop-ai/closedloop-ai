...
export function CreateCustomFieldDialog({
  ...
  const onSubmit = (values: FormValues) => {
    // Fire-and-forget: react-hook-form's handleSubmit re-throws a rejected
    // async onValid handler, which would surface as an unhandled rejection
    // (React does not await it). The failure is already toasted by the global
    // mutation onError handler, so close the dialog only on success.
    if (isEditMode && field) {
