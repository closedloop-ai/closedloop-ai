...
// vi.mock is hoisted above these imports by Vitest, so the imported
// useUpdateProject resolves to the mock below.
...
describe("EditableProjectDescription", () => {
  ...
  describe("Cancel behavior", () => {
    ...
    it("does not save when Escape is pressed while focused", () => {
      // handleCancel blurs the focused field; the resulting onBlur=>save must
      // not commit the edited value (regression: Escape silently saved).
      const { container } = render(
        <EditableProjectDescription
          initialDescription="Original"
          projectId={projectId}
