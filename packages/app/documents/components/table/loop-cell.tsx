...
const LOOP_CELL_BASE_CLASS =
  "flex h-full min-h-11 w-[124px] shrink-0 items-center gap-1.5 border-l px-3 py-2";
const LOOP_CELL_LINK_CLASS = `${LOOP_CELL_BASE_CLASS} hover:bg-muted/50`;

function LoopCellDash() {
  return (
    <div className="flex h-full min-h-11 w-[124px] shrink-0 items-center border-l px-3 py-2">
  ...
  ({ loopId, children, onClick, ...rest }, ref: Ref<HTMLAnchorElement>) => {
    ...
    return (
      <Link
        ...
        href={buildOrgPath(`/loops/${loopId}`)}
...
function DefaultLoopCell({ item }: { item: DocumentRowItem }) {
  const { activeLoops, activeLoopsByDocumentId } = useContext(RowEditContext);
  const documentId = item.data.id;

  const genStatus =
    item.kind === "document" ? item.data.generationStatus : undefined;
  const isFailed = genStatus?.status === "FAILURE";

  const loop =
    activeLoopsByDocumentId?.get(documentId) ??
    activeLoops?.find((l) => l.documentId === documentId);
...
function TeamLoopCell({ item }: { item: DocumentRowItem }) {
  const { activeLoops, activeLoopsByDocumentId, loopSummaries } =
    useContext(RowEditContext);
  const documentId = item.data.id;
