...
import { useBatchMoveArtifacts } from "@repo/app/documents/hooks/use-artifact-links";
      ...
      const results = await Promise.allSettled(
        entitiesToMove.map((item) =>
          batchMove.mutateAsync(
            ...
            {
              // Suppress the per-item default error toast so a batch failure
              // surfaces as a single aggregated summary toast below, not N
              // toasts. A per-call onError overrides the global default.
              onError: () => undefined,
            }
          )
        )
