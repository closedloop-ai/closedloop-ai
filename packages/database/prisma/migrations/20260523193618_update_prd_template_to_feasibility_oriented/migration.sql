-- Update all PRD template documents with the revised PRD template content.
-- Creates a new document version (preserving version history) rather than
-- overwriting prior content.
--
-- Pairs with the prompt and template changes in this PR:
--   apps/api/app/documents/template-seeds.ts (PRD_TEMPLATE source-of-truth)
--   apps/api/lib/loops/prompts.ts            (GENERATE_PRD instructions)
--
-- The new Technical Considerations section is scoped to feasibility,
-- constraints, and dependencies. The legacy "Architecture" placeholder and
-- library-level "System Dependencies" / "Data migrations" hints are removed.
-- Replacements include "Existing capabilities this depends on", reframed
-- "Compatibility / Rollout Coordination", and "Feasibility Risks".
--
-- Org-customized templates: any organization that has edited its PRD
-- template will get a new "latest" version layered on top of its
-- customization. Prior versions are preserved in document_versions and can
-- be restored from version history if a customer requests it.
--
-- Dry-run (run this SELECT before deploying to confirm row counts):
--   SELECT COUNT(*) FROM document_detail WHERE template_for_type = 'PRD';

-- Step 1: Bump latest_version on every PRD template document_detail row.
UPDATE document_detail
SET latest_version = latest_version + 1
WHERE template_for_type = 'PRD';

-- Step 2: Touch the parent artifact's updated_at so the refresh shows up in
-- recently-edited sorts.
UPDATE artifacts
SET updated_at = NOW()
WHERE id IN (
  SELECT artifact_id FROM document_detail WHERE template_for_type = 'PRD'
);

-- Step 3: Insert a new document_versions row carrying the updated content.
-- The content below MUST stay in sync with PRD_TEMPLATE in
-- apps/api/app/documents/template-seeds.ts.
INSERT INTO document_versions (id, artifact_id, version, content, created_at)
SELECT
  gen_random_uuid(),
  dd.artifact_id,
  dd.latest_version,
  $prd_template_v2$# Product Requirements Document
...
WHERE dd.template_for_type = 'PRD';
