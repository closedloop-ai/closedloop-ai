/**
 * Gateway probe -- detects a running Closedloop Desktop gateway.
 *
 * Scans localhost ports for a gateway health endpoint and parses
 * the response. Framework-agnostic: uses only fetch and zod.
 */

import { z } from "zod";
import type { GatewayDetectionState } from "./types";

type GatewayProbeResult = Pick<
...
export function getPossibleGatewayHostnames(): {
  hostname: string;
  port: number;
}[] {
...
export async function probeGateway(): Promise<GatewayProbeResult> {
  for (const { hostname, port } of getPossibleGatewayHostnames()) {
