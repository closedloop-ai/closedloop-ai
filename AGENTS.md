...
- Polling query hooks must stop polling on every terminal status for the workflow, including failure or expiry states, not only the success state. They must also stop or deliberately back off when `query.state.status === "error"` so missing resources or server errors do not create infinite retry loops.
