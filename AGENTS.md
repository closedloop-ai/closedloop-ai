...
- When expanding a canonical error/status label map to new UI surfaces, keep badge styling and fallback behavior in the same change. Add a regression test for at least one newly covered or otherwise unmapped code so missing style coverage cannot silently render an unstyled state.
- Structured error `details` payloads must keep field names aligned with value cardinality and consumer expectations. Use plural field names for arrays, or convert to a scalar before storing it under a singular field name; include a focused test for the emitted shape when the payload crosses an API/app/package boundary.
- When layering specialized error classifications on top of canonical error codes, preserve canonical-code precedence. Category-specific overrides should apply only to the intended generic or unknown code family, and tests should cover a known non-target code carrying matching category metadata.
