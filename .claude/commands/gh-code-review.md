...
```bash
# Search patch content for CI runner paths
# Look for /home/runner/, /github/workspace/, etc. in FILE_PATCHES values
```

**Check 2: Path leakage** — absolute machine-specific paths in new files:
```bash
# Search patch content for /Users/, /home/, C:\ patterns
...
- **File-level findings** (e.g., `.dev-environment.json` shouldn't be committed): Use `line: 1` of the file as the comment anchor
- **Cross-file findings** (e.g., gitignore drift for `.gitignore`): If `.gitignore` is in the PR diff, comment on its last changed line. If NOT in the diff, mark as `"inline": false` (summary-only)
...
Store hygiene findings in the same format as agent findings:
...
```json
{
  ...
  "explanation": "This file was generated by CI and contains machine-specific paths. It should not be committed to the repository.",
