...
### API & Service Layer
- **[convention]**: For admin-only routes, use `isOrgAdmin(clerkOrgId, clerkUserId)` from `@/lib/auth/org-admin` instead of duplicating the `ADMIN_ROLES` set. The canonical admin role set is defined once in `apps/api/lib/auth/org-admin.ts`. (context: auth|admin-roles|org-admin|DRY)
- **[convention]**: Unauthenticated public endpoints must have IP-based rate limiting via `rateLimit()` from `@repo/security`. Pattern: `await rateLimit(\`prefix_\${request.headers.get("x-forwarded-for") ?? "unknown"}\`, 30, "60s", request)`. Without rate limiting, attackers can drive sustained DB load with no throttle. (context: security|rate-limiting|public-endpoints|unauthenticated)
- **[mistake]**: Always wrap `navigator.clipboard.writeText()` in try-catch. The clipboard API rejects when permissions are denied (HTTP contexts, Firefox, user block). Without try-catch, the rejection becomes an unhandled promise rejection with no user feedback. (context: clipboard|error-handling|browser-api|try-catch)
...
- **[mistake]**: When a mutation returns updated data (e.g., generating a new token returns the token+URL), use `queryClient.setQueryData()` in `onSuccess` to update the cache immediately — don't just `invalidateQueries()`. Invalidation triggers a refetch, but there's a window where the UI shows stale data (e.g., old revoked URL still copyable). Use `setQueryData` for instant cache updates, especially when the mutation response contains the new state. (context: tanstack-query|mutations|cache-update|stale-data|setQueryData)
- **[mistake]**: When using `fetch()` directly in `queryFn` (e.g., unauthenticated public endpoints), always check `response.ok` before calling `response.json()`. Non-JSON error responses (502 HTML pages, CDN rate limits) throw confusing SyntaxError instead of a meaningful HTTP error. Pattern: `if (!response.ok) { throw new Error(\`HTTP \${response.status}\`); }` (context: tanstack-query|fetch|error-handling|response-ok)
