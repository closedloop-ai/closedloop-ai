...
- **[pattern]**: `withDb.tx` propagates its transaction via `AsyncLocalStorage`, so nested `withDb` and `withDb.tx` calls automatically reuse the outer transaction. This matches the pattern used by `withImplicitTransaction`. (context: database|transactions|withDb|connection-pool|prisma)
