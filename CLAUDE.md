...
- **Never hand-write migration SQL files** — always let `prisma migrate dev` generate them. Hand-written migrations miss Prisma's drift detection, FK cleanup, and standard formatting. If you need to run the command, first update the schema, then run the migrate command and let Prisma diff the schema against the database.
...
- **[mistake]**: Never hand-write migration SQL files (creating `migration.sql` directly). Always run `prisma migrate dev` and let Prisma generate the SQL by diffing the schema against the database. Hand-written migrations miss drift detection, FK cleanup, and cause `_prisma_migrations` tracking issues. (context: prisma|migrations|hand-written|drift|generation)
