...
- **[pattern]**: When creating new TanStack Query hooks in `apps/app/hooks/queries/`, always: (1) export a `queryKeys` factory with `.all` and `.detail(id)` pattern, (2) use retry function to skip 404s if resource may not exist yet, (3) add cache invalidation to related mutations. (context: tanstack-query|hooks|query-keys|patterns)
