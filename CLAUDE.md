...
- **[pattern]**: In this codebase's Claude CLI stream handlers (`processStreamEvent`, `processClaudeStreamLine`), result events must: (1) capture session_id for ALL results including errors, (2) check `is_error` independently before `subtype`, (3) fire `onResultEvent` for ALL result types (success and error). Reference implementation is in the codex review route. (context: claude-cli|stream-events|control-flow|processStreamEvent)
