...
/**
 * Matches reserved browser-key internal paths across current and legacy
 * Desktop API namespaces. Public command entry points must reject these paths.
 */
export function isReservedBrowserKeyRevocationPath(path: string): boolean {
  const normalizedPath = normalizeDesktopApiPath(toPathname(path));
  return (
    normalizedPath !== null && RESERVED_BROWSER_KEY_PATHS.has(normalizedPath)
  );
}
