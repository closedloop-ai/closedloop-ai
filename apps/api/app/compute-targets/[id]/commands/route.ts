...
function isPathInNamespace(
  path: string,
  namespace: DesktopApiNamespace
): boolean {
  return path.startsWith(getDesktopApiPrefix(namespace));
}
...
async function resolveCommandSigningContext(input: {
  ...
  const namespace =
    getDesktopApiNamespaceFromCapabilities(input.capabilities) ??
    CURRENT_DESKTOP_API_NAMESPACE;
  const serverSigningSupported = await isComputeTargetSigningSupportedForUser({
  ...
  return {
    ...
    effectiveCommandSigning:
      hasDesktopCommandSigningEnforcement(input.capabilities) &&
      serverSigningSupported,
  };
}
...
function validateSigningCompatibility(
  ...
  if (
    signatureFields &&
    !isPathInNamespace(input.path, signingContext.namespace)
  ) {
    return NextResponse.json(
      {
        success: false,
...
function buildCommandInputForDispatch(
  ...
  return signatureFields
    ...
    : {
        ...unsignedInput,
        path: rewriteDesktopApiPath(unsignedInput.path, namespace),
      };
}
