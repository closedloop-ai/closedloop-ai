...
describe("GET /users/[id]/stats startDate boundary (FEA-4064)", () => {
  ...
  it("parses a valid startDate query param and forwards it to the org-scoped service", async () => {
    ...
    const [userId, organizationId, startDate] =
      mocks.getUserStats.mock.calls[0];
  ...
  it("rejects a malformed startDate with 400 before touching the service", async () => {
    const response = await GET(
      request(
        "https://api.example.test/users/user-2/stats?startDate=not-a-date"
      ),
      routeContext("user-2")
    );

    expect(response.status).toBe(400);
    // Fail fast at the boundary: the stats aggregation must not run for a bad
    // window, and the existence lookup is likewise short-circuited.
  ...
  it("omits startDate (all-time totals) when the query param is absent", async () => {
    const response = await GET(
      request("https://api.example.test/users/user-2/stats"),
      routeContext("user-2")
    );

    expect(response.status).toBe(200);
    expect(mocks.getUserStats).toHaveBeenCalledWith(
      "user-2",
      "org-1",
      undefined
    );
  ...
  it("returns 404 when the target user is not in the viewer's org", async () => {
    ...
    const response = await GET(
      request("https://api.example.test/users/ghost/stats"),
      routeContext("ghost")
    );
