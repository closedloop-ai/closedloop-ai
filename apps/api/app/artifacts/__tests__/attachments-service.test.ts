 ...
 *   - listByArtifact returns records with createdAt serialized as ISO strings
 ...
 *   - All methods throw "Artifact not found" when the ownership check returns null
 */
...
describe("attachmentsService.listByArtifact", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("returns records with createdAt serialized as ISO 8601 strings", async () => {
    ...
    const results = await attachmentsService.listByArtifact(
      ARTIFACT_ID,
      ORG_ID
    );
  ...
  it("throws 'Artifact not found' when artifact ownership check returns null", async () => {
    ...
    await expect(
      attachmentsService.listByArtifact("nonexistent-artifact", ORG_ID)
    ).rejects.toThrow("Artifact not found");
  });
});
