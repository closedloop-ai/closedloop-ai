...
describe("results[] round-trip fidelity through Prisma JSON column", () => {
  ...
  it("returns results[] structurally identical when retrieved via getEventsPaginated (round-trip)", async () => {
    // Simulate the round-trip: addEvent stores results[], then getEventsPaginated
    // returns data with the results[] intact (simulating Prisma JSON column read).
    const results = [
      {
        status: "success" as const,
        fullName: "org/repo",
    ...
    // The DB stores the event data as JSON and returns it as-is.
    // Simulate what getEventsPaginated returns after a Prisma JSON round-trip.
    ...
    mockLoopsService.getEventsPaginated.mockResolvedValue({
      data: [storedEventData],
      total: 1,
    });

    const response = await loopsService.getEventsPaginated("loop-1", "org-1", {
      type: "completed",
    });

    expect(response.data).toHaveLength(1);
    const returnedEvent = response.data[0] as typeof storedEventData;
    expect(returnedEvent.results).toEqual(results);

    // Structural identity: each field must match exactly
    expect(returnedEvent.results[0]).toEqual({
      status: "success",
      fullName: "org/repo",
      prUrl: "https://github.com/org/repo/pull/99",
      prNumber: 99,
    ...
    expect(returnedEvent.results[1]).toEqual({
      status: "skipped",
      fullName: "org/other-repo",
      reason: "no_changes",
    });
