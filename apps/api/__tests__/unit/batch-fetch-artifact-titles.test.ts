/**
 * Unit tests for artifactsService.batchFetchArtifactTitles method.
 *
...
describe("artifactsService.batchFetchArtifactTitles", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("slugs that exist returns correct mapping", async () => {
    ...
    const result = await artifactsService.batchFetchArtifactTitles("org-1", [
      "prd-abc",
      "plan-xyz",
    ]);
  ...
  it("input exceeding max slugs throws", async () => {
    ...
    await expect(
      artifactsService.batchFetchArtifactTitles("org-1", slugs)
    ).rejects.toThrow("batchFetchArtifactTitles: too many slugs");
