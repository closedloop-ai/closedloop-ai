...
import {
  failLoopFromRejectedCommand,
  failLoopFromTerminalCommandError,
} from "@/lib/loops/rejected-command-loop-failure";
...
function mockScopedLoopCommand(overrides?: {
  ...
  mockDb.desktopCommand.findUnique.mockResolvedValue({
    ...
    computeTarget: { organizationId: ORG_ID },
...
describe("failLoopFromTerminalCommandError", () => {
  it("fails symphony_loop with the terminal desktop error message", async () => {
    const message =
    ...
    const result = await failLoopFromTerminalCommandError({
      commandId: COMMAND_ID,
      targetId: TARGET_ID,
      error: message,
    });
    ...
    expect(mockDb.loop.findFirst).toHaveBeenCalledWith({
      where: {
        id: LOOP_ID,
        organizationId: ORG_ID,
        computeTargetId: TARGET_ID,
  ...
  it("trims and caps terminal desktop error messages before persistence", async () => {
    const message = `  ${"x".repeat(2100)}  `;
    const cappedMessage = "x".repeat(2000);
  ...
  it("uses a safe fallback for blank terminal desktop error messages", async () => {
    const result = await failLoopFromTerminalCommandError({
      commandId: COMMAND_ID,
      targetId: TARGET_ID,
      error: "   ",
    ...
    expect(mockDb.loop.updateMany).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          error: {
            code: LoopErrorCode.ProcessFailed,
            message: "Desktop loop command failed",
          },
        }),
      })
    );
  ...
  it("does not fail loop kill command errors", async () => {
    mockScopedLoopCommand({ operationId: "symphony_loop_kill" });
