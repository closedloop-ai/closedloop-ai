...
/**
 * PLN-1678: `next.config.ts` sets `typescript.ignoreBuildErrors`, so the build
 * no longer type-checks `apps/api`. Next 16.3 type-checks with the TypeScript
 * CLI, which covers the whole tsconfig project including this app's tests — two
 * of which import trees the containerized E2E api image does not COPY, so the
 * build failed there on TS2307 while a full checkout was clean.
 *
 * That is only safe while something else checks the SAME file set, including
 * the generated `.next/types/validator.ts` that validates every route-handler
 * signature against its route's `params`. `tsconfig.json` globs
 * `.next/types/**\/*.ts`, but that glob matches nothing in a cold checkout, so
 * the required `typecheck` job only sees the validator because its script runs
 * `next typegen` first. The legs are load-bearing together and useless apart;
 * this test is that guard, and mirrors apps/app's (ISS-5133).
 */

const API_ROOT = join(import.meta.dirname, "..");
const NEXT_CONFIG_PATH = join(API_ROOT, "next.config.ts");
const PACKAGE_JSON_PATH = join(API_ROOT, "package.json");
const TSCONFIG_PATH = join(API_ROOT, "tsconfig.json");
...
function readTypecheckScript(): string {
  ...
  if (!script) {
    throw new Error("apps/api/package.json must define a `typecheck` script");
  }
  return script;
}
...
describe("route-type validation has an owner outside the build", () => {
  ...
  it("keeps the generated route types in the tsc program", () => {
    // typegen can run and `tsc` can pass while covering nothing, if this glob
    // ever leaves `include` — the generated files would simply not be part of
    // the program, silently and greenly.
