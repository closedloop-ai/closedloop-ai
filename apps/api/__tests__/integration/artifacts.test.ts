...
describe.skipIf(!hasDatabase)("Artifacts Service Integration", () => {
  ...
  it("creates multiple artifacts each with latestVersion 1", async () => {
    await autoRollbackTransaction(async () => {
      ...
      const a1 = await artifactsService.create(testOrgId, testUserId, {
        projectId: testProjectId,
        type: "PRD",
        title: "My Feature",
      ...
      const a2 = await artifactsService.create(testOrgId, testUserId, {
        projectId: testProjectId,
        type: "PRD",
  ...
  it("createNewVersion creates a new version of artifact content", async () => {
    await autoRollbackTransaction(async () => {
      ...
      const updated = await artifactsService.createNewVersion(
        original!.id,
        testOrgId,
        testUserId,
  ...
  it("findAll filters by type", async () => {
    await autoRollbackTransaction(async () => {
      ...
      await artifactsService.create(testOrgId, testUserId, {
        projectId: testProjectId,
        type: "PRD",
        title: "Test PRD",
        content: "PRD content",
