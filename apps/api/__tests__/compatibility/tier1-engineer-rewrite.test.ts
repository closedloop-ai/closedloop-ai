...
describe("EXACT_OPERATION_IDS", () => {
  it("maps /api/engineer/health-check to health_check", () => {
    expect(EXACT_OPERATION_IDS["/api/engineer/health-check"]).toBe(
      "health_check"
    );
  });

  it("maps /api/engineer/symphony/launch to symphony_launch", () => {
    expect(EXACT_OPERATION_IDS["/api/engineer/symphony/launch"]).toBe(
      "symphony_launch"
    );
  });

  it("maps /api/engineer/symphony/kill to symphony_kill", () => {
    expect(EXACT_OPERATION_IDS["/api/engineer/symphony/kill"]).toBe(
      "symphony_kill"
    );
  });

  it("maps /api/engineer/symphony/loop to symphony_loop", () => {
    expect(EXACT_OPERATION_IDS["/api/engineer/symphony/loop"]).toBe(
      "symphony_loop"
    );
  });

  it("maps /api/engineer/repos to repos_config", () => {
    expect(EXACT_OPERATION_IDS["/api/engineer/repos"]).toBe("repos_config");
  });

  it("maps /api/engineer/directories to filesystem", () => {
    expect(EXACT_OPERATION_IDS["/api/engineer/directories"]).toBe("filesystem");
  });
...
describe("PREFIX_OPERATION_IDS", () => {
  it("maps /api/engineer/symphony/status/ prefix to symphony_status", () => {
    const entry = PREFIX_OPERATION_IDS.find(
      ([prefix]) => prefix === "/api/engineer/symphony/status/"
    );
    expect(entry).toBeDefined();
    expect(entry?.[1]).toBe("symphony_status");
  });

  it("maps /api/engineer/symphony/chat/ prefix to symphony_chat", () => {
    const entry = PREFIX_OPERATION_IDS.find(
      ([prefix]) => prefix === "/api/engineer/symphony/chat/"
    );
    expect(entry).toBeDefined();
    expect(entry?.[1]).toBe("symphony_chat");
  });

  it("maps /api/engineer/codex/ prefix to codex_review", () => {
    const entry = PREFIX_OPERATION_IDS.find(
      ([prefix]) => prefix === "/api/engineer/codex/"
    );
    expect(entry).toBeDefined();
    expect(entry?.[1]).toBe("codex_review");
  });

  it("maps /api/engineer/git prefix to git_action (less-specific git prefix)", () => {
    const entry = PREFIX_OPERATION_IDS.find(
      ([prefix]) => prefix === "/api/engineer/git"
    );
    expect(entry).toBeDefined();
    expect(entry?.[1]).toBe("git_action");
  });

  it("orders /api/engineer/codex/argue/ before /api/engineer/codex/ (more-specific first)", () => {
    const argueIndex = PREFIX_OPERATION_IDS.findIndex(
      ([prefix]) => prefix === "/api/engineer/codex/argue/"
    );
...
describe("resolveOperationId", () => {
  it("returns null for paths that do not start with /api/engineer/", () => {
  ...
  it("resolves exact match: /api/engineer/symphony/launch → symphony_launch", () => {
    expect(resolveOperationId("/api/engineer/symphony/launch")).toBe(
      "symphony_launch"
    );
  });

  it("resolves exact match: /api/engineer/repos → repos_config", () => {
    expect(resolveOperationId("/api/engineer/repos")).toBe("repos_config");
  });

  it("resolves prefix match: /api/engineer/symphony/status/<id> → symphony_status", () => {
    expect(
      resolveOperationId("/api/engineer/symphony/status/run-abc-123")
    ).toBe("symphony_status");
  });
  ...
  it("resolves prefix match: /api/engineer/codex/argue/<id> → codex_argue", () => {
    expect(resolveOperationId("/api/engineer/codex/argue/some-id")).toBe(
      "codex_argue"
    );
  });

  it("resolves prefix match: /api/engineer/codex/<id> → codex_review (not codex_argue)", () => {
    expect(resolveOperationId("/api/engineer/codex/some-id")).toBe(
      "codex_review"
    );
  });

  it("resolves prefix match: /api/engineer/git/pr/123 → git_pr", () => {
    expect(resolveOperationId("/api/engineer/git/pr/123")).toBe("git_pr");
  });

  it("resolves prefix match: /api/engineer/git/commit → git_action (less-specific git prefix)", () => {
    expect(resolveOperationId("/api/engineer/git/commit")).toBe("git_action");
  });

  it("returns null for unknown /api/engineer/ subpath", () => {
    expect(resolveOperationId("/api/engineer/unknown-operation")).toBeNull();
  });

  it("exact match takes precedence over prefix match for /api/engineer/symphony/loop", () => {
    expect(resolveOperationId("/api/engineer/symphony/loop")).toBe(
      "symphony_loop"
    );
  });
});
