...
import { POST } from "@/app/documents/[id]/execute/route";
...
describe("POST /api/artifacts/[id]/execute", () => {
  ...
  it("executes implementation plan successfully", async () => {
    ...
    vi.mocked(documentsService.executeImplementationPlan).mockResolvedValue({
      success: true,
      correlationId,
    });
    ...
    expect(documentsService.executeImplementationPlan).toHaveBeenCalledWith(
      artifactId,
      "org-123",
      "user-123"
    );
  ...
  it("returns 500 on service exception", async () => {
    vi.mocked(documentsService.executeImplementationPlan).mockRejectedValue(
