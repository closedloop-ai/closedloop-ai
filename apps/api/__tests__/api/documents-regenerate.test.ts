...
import { POST } from "@/app/documents/[id]/regenerate/route";
...
describe("POST /api/artifacts/[id]/regenerate", () => {
  ...
  it("dispatches to regenerateImplementationPlan for plans", async () => {
    ...
    vi.mocked(documentsService.findByIdSimple).mockResolvedValue(
      mockArtifact as any
    );
    vi.mocked(documentsService.regenerateImplementationPlan).mockResolvedValue({
      success: true,
      document: mockArtifact as any,
    });
    ...
    expect(documentsService.regenerateImplementationPlan).toHaveBeenCalledWith(
      artifactId,
      "org-123",
      "user-123"
    );
    expect(documentsService.generatePRD).not.toHaveBeenCalled();
  });
  ...
  it("dispatches to generatePRD for PRD artifacts", async () => {
    ...
    vi.mocked(documentsService.generatePRD).mockResolvedValue({
      success: true,
      document: mockArtifact as any,
    });
    ...
    const json = await response.json();
    expect(json.success).toBe(true);
    expect(documentsService.generatePRD).toHaveBeenCalledWith(
      artifactId,
      "org-123",
      "user-123",
      null
    ...
    expect(
      documentsService.regenerateImplementationPlan
  ...
  it("returns 404 when artifact not found", async () => {
    vi.mocked(documentsService.findByIdSimple).mockResolvedValue(null);
  ...
  it("returns 500 on service exception", async () => {
    ...
    vi.mocked(documentsService.regenerateImplementationPlan).mockRejectedValue(
