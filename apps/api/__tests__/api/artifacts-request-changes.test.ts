...
import { POST } from "@/app/artifacts/[id]/request-changes/route";
...
describe("POST /api/artifacts/[id]/request-changes", () => {
  ...
  it("requests plan changes successfully", async () => {
    ...
    vi.mocked(artifactsService.requestPlanChanges).mockResolvedValue({
      success: true,
    ...
    expect(artifactsService.requestPlanChanges).toHaveBeenCalledWith(
      artifactId,
      "org-123",
      "user-123",
  ...
  it("returns 500 on service exception", async () => {
    vi.mocked(artifactsService.requestPlanChanges).mockRejectedValue(
