// Prototype-local session detail fixtures. Presentational only.
...
export type TraceTurn = {
  ...
  kind?: "idle";
  side: "human" | "agent";
  /** Human source whose session produced this turn. */
  actorId?: string;
  actorName?: string;
...
export type TimelineCostEvent = {
  atMinutes: number;
  /** Human source whose session produced this cost. */
  actorId?: string;
  actorName?: string;
  actorInitials?: string;
  model: string;
...
const COMMENTS: SessionComment[] = [
  {
    ...
    author: "Parker Byrd",
    at: "34m ago",
    anchorPreview: "Match the existing patterns in",
