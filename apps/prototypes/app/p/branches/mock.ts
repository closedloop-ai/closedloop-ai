...
import type { PrototypeFileCoverage } from "./file-coverage-fixtures";
...
// Consumer-visible data state for a Branches row (mirrors the product
// `BranchDataState`). Optional on the row so older producers that omit it fall
// back to the ordinary presentation. Only `awaiting_sync` changes the render.
...
export type BranchRow = {
  ...
  tags: string[];
  /**
   * Optional cloud-derived data state. Omitted on most rows so they take the
   * ordinary presentation; `awaiting_sync` marks a row whose data is still
   * settling, surfaced by a loading indicator in place of the branch icon.
   */
...
export const branchRows: readonly BranchRow[] = [
  {
    ...
    prNumber: 1296,
    prTitle: "Reindex embeddings store",
    prUrl: "https://github.com/closedloop-ai/symphony-alpha/pull/1296",
    ...
    sessionCount: 3,
    commentCount: null,
    lastActivityAt: "2026-07-28T18:48:00Z",
    lastActivityLabel: "12m ago",
    collaborators: ["Alex Rivera", "Sam Chen"],
    tags: ["search", "backend"],
  ...
  {
    id: "br_files_zero",
    branchName: "agent/files-empty-fixture",
    baseBranch: "main",
    repo: "closedloop-ai/symphony-alpha",
    owner: "Alex Rivera",
    status: BranchStatus.Open,
    provenance: Provenance.Agent,
    prNumber: 1297,
    prTitle: "Verify empty selected-PR file evidence",
    prUrl: "https://github.com/closedloop-ai/symphony-alpha/pull/1297",
    prState: PrState.Open,
    checksPassed: 8,
    checksTotal: 8,
    additions: 0,
    ...
    sessionCount: 4,
    commentCount: null,
    lastActivityAt: "2026-07-28T10:30:00Z",
  ...
  {
    id: "br_unpriced_sessions",
    branchName: "agent/unpriced-session-evidence",
    baseBranch: "main",
    repo: "closedloop-ai/symphony-alpha",
    owner: "Sam Chen",
    status: BranchStatus.Open,
    provenance: Provenance.Agent,
    prNumber: 1298,
    prTitle: "Preserve activity when Session cost is unavailable",
    prUrl: "https://github.com/closedloop-ai/symphony-alpha/pull/1298",
    prState: PrState.Open,
    checksPassed: 8,
    checksTotal: 8,
    additions: 48,
...
/** Availability of canonical attributed cost evidence for one projection. */
export const BranchCostAvailability = {
  Complete: "complete",
  Partial: "partial",
  Unavailable: "unavailable",
} as const;
export type BranchCostAvailability =
  (typeof BranchCostAvailability)[keyof typeof BranchCostAvailability];
...
export type PrComment = {
  ...
  body: string;
  provider?: PrCommentProviderProvenance;
  replies?: PrComment[];
};
/** Provider comment evidence mirrored from the shared API contract. */
export const PrCommentProviderKind = {
  Issue: "issue",
  Review: "review",
  ReviewReply: "review_reply",
} as const;

export type PrCommentProviderKind =
  (typeof PrCommentProviderKind)[keyof typeof PrCommentProviderKind];

export const PrCommentsState = {
  OverLimitTruncated: "over_limit_truncated",
  StaleMixed: "stale_mixed",
} as const;

export type PrCommentsState =
  (typeof PrCommentsState)[keyof typeof PrCommentsState];

export type PrCommentProviderProvenance = {
  bodyTruncated: boolean;
  kind: PrCommentProviderKind;
  line?: number;
  login: string;
  path?: string;
  providerUrl?: string;
...
/** Independent bounded-availability facts shown by the prototype. */
export type PrCommentsAvailability = {
...
export type SessionLane = {
  ...
  /** Captured Session cost before global Branch attribution. */
  rawCostUsd?: number | null;
  /** Canonical per-Branch share; consumers must not divide it again. */
...
export type TimelineColumn = {
  ...
  segments: {
    color: string;
    pct: number;
    /** Stable lane identity when timing evidence can name its source Session. */
    sessionId?: string;
  }[];
};
...
export type BranchDetail = {
  ...
  deletions: number | null;
  costLabel: string;
  /** Captured lifetime Session subtotal before shared-Branch attribution. */
  rawCostUsd: number | null;
  /** Canonical lifetime Branch share; consumers must not divide it again. */
  attributedCostUsd: number | null;
  /** Completeness of the attributed subtotal across qualifying Sessions. */
  costAvailability: BranchCostAvailability;
  /** Local explanation required for partial or unavailable cost evidence. */
  ...
  files: FileChange[];
  /** Mirrors selected-PR response coverage, counts, and gross-total availability. */
  fileCoverage: PrototypeFileCoverage;
...
/** Atomic selected-PR projection so identity and description cannot diverge. */
