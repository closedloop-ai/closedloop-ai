...
function makeVariantScenario(
  ...
  return {
    ...
    sessions,
    trace,
    timelineColumns: addStableSessionIds(
      [
...
function addStableSessionIds(
  columns: BranchScenario["timelineColumns"],
  sessions: readonly SessionLane[]
): BranchScenario["timelineColumns"] {
  const sessionIdsByColor = new Map<string, string[]>();
  for (const { color, id } of sessions) {
    const sessionIds = sessionIdsByColor.get(color) ?? [];
    sessionIds.push(id);
    sessionIdsByColor.set(color, sessionIds);
  }
  return columns.map((column) => ({
    ...column,
    segments: column.segments.map((segment) => {
      const sessionIds = sessionIdsByColor.get(segment.color);
      const sessionId = sessionIds?.length === 1 ? sessionIds[0] : undefined;
      return sessionId ? { ...segment, sessionId } : segment;
    }),
  }));
}
