...
const NavLink = ({
  ...
  // A nav item with an onClick behaves like a button (used for the Agents item,
  // which navigates back to the parent page from any subpage).
  if (onClick) {
    return (
      <SidebarMenuItem>
        <SidebarMenuButton
        ...
        <SidebarMenu className="gap-0">
          {artifactsNav.map((item) => (
            <NavLink
              item={item}
              key={item.label}
              onClick={item.icon === "agents" ? onNavigateAgents : undefined}
            />
          ))}
        </SidebarMenu>
