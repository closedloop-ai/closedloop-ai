...
export function BranchHeadlineCards({ detail }: { detail: BranchDetail }) {
  return (
    <div className="mt-8 grid grid-cols-1 gap-3 sm:grid-cols-2">
      <MetricCard
        detail={`${(detail.additions ?? 0) + (detail.deletions ?? 0)} lines changed · ${detail.costLabel}`}
        info={{
          what: "Total lines changed (added + removed) per dollar spent.",
          how: "Lines changed divided by estimated cost. Prefers the connected PR's live LOC.",
        }}
        label="Value per $"
        unitLabel="LOC/$"
...
export function BranchDeliveredPanel({ detail }: { detail: BranchDetail }) {
  ...
  const stateChip =
    detail.prState == null ? null : (
      <Chip size="sm" variant={PR_STATE_VARIANT[detail.prState]}>
        {detail.prState}
      </Chip>
    );
  ...
  return (
    <section>
      ...
      <div className="rounded-md border bg-card px-3.5 py-3">
        ...
        {detail.prTitle ? (
          <p className="font-medium text-sm">{detail.prTitle}</p>
        ) : null}
        {detail.prBody ? (
          <>
            <div
              className={cn(
                "mt-2 whitespace-pre-wrap break-words text-foreground text-sm leading-relaxed",
                !open &&
...
export function BranchFilesChangedPanel({ detail }: { detail: BranchDetail }) {
  const totalAdd = detail.files.reduce((sum, file) => sum + file.additions, 0);
  const totalDel = detail.files.reduce((sum, file) => sum + file.deletions, 0);
    ...
    <section>
        ...
        count={
          <span className="flex items-center gap-2">
            ...
            <span className="font-mono">
              <b className="text-success">+{totalAdd}</b>{" "}
              <b className="text-destructive">−{totalDel}</b>
            </span>
          </span>
        }
        title="Files changed"
      ...
      <div className="flex flex-col">
        {detail.files.map((file) => (
          <div className="flex h-8 items-center gap-2.5 px-2" key={file.path}>
            <FileIcon
              aria-hidden
              className="size-3.5 shrink-0 text-muted-foreground"
            />
