...
import { buildSessionComments, getPrComments } from "./mock-comments";
...
/**
 * Slice a scenario to `count` sessions and keep the legend and timeline bars
 * consistent with that slice — so nothing on the tab claims a session count the
 * content below it contradicts. The trace is the conversation transcript for
 * the branch, kept whole.
 */
function reconcileScenario(scenario: BranchScenario, count: number) {
  ...
  return { sessions, trace, legend, columns, eventDots };
}
 ...
 */
export function buildBranchDetail(row: BranchRow): BranchDetail {
  const merged = row.status === BranchStatus.Merged;
  const scenario = SCENARIOS[row.id] ?? SEED_SCENARIO;
  const count = SCENARIOS[row.id]
    ? scenario.sessions.length
    : clamp(row.sessionCount, 0, scenario.sessions.length);
  const { sessions, trace, legend, columns, eventDots } = reconcileScenario(
    scenario,
    count
  );
  ...
  return {
    ...
    sessionComments: buildSessionComments(row.id, sessions, trace),
    sessions,
    timeline: {
      legend,
      columns,
