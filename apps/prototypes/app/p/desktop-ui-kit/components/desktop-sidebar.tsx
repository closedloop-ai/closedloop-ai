"use client";

import { Input } from "@repo/design-system/components/ui/input";
import {
  Sidebar,
  SidebarContent,
  SidebarFooter,
  SidebarGroup,
  SidebarMenu,
  SidebarMenuButton,
  SidebarMenuItem,
  SidebarNavLinkItem,
} from "@repo/design-system/components/ui/sidebar";
import { SidebarCollapsibleSection } from "@repo/design-system/components/ui/sidebar-collapsible-section";
import { SidebarCountBadge } from "@repo/design-system/components/ui/sidebar-count-badge";
import {
  BarChart3Icon,
  BotIcon,
  ChevronsUpDownIcon,
  ClipboardListIcon,
  GitBranchIcon,
  HistoryIcon,
  InboxIcon,
  LayoutDashboardIcon,
  type LucideIcon,
  SearchIcon,
  SettingsIcon,
  ShieldIcon,
  StethoscopeIcon,
} from "lucide-react";
import { labsNav, mainNav, type NavIconName, type NavItem } from "../mock";

const navIcons: Record<NavIconName, LucideIcon> = {
  dashboard: LayoutDashboardIcon,
  sessions: HistoryIcon,
  branches: GitBranchIcon,
  agents: BotIcon,
  insights: BarChart3Icon,
  plans: ClipboardListIcon,
  approvals: ShieldIcon,
  requests: InboxIcon,
  diagnostics: StethoscopeIcon,
  settings: SettingsIcon,
};

// Decorative macOS window controls. On the real desktop build macOS hides the
// native title bar and draws the stoplights over this reserved strip; here we
// render static dots so the browser prototype reads as the Electron window.
const MacStoplights = () => (
  <div className="flex h-7 shrink-0 items-center gap-2 px-3.5 pt-1">
    <span className="size-3 rounded-full bg-[#ff5f57] ring-1 ring-black/10 ring-inset" />
    <span className="size-3 rounded-full bg-[#febc2e] ring-1 ring-black/10 ring-inset" />
    <span className="size-3 rounded-full bg-[#28c840] ring-1 ring-black/10 ring-inset" />
  </div>
);

const NavLink = ({ item }: { item: NavItem }) => {
  const Icon = navIcons[item.icon];
  return (
    <SidebarNavLinkItem
      className="text-sm"
      icon={<Icon />}
      isActive={item.isActive}
      key={item.label}
      title={item.label}
      tooltip={item.label}
      trailing={
        item.count === undefined ? undefined : (
          <SidebarCountBadge count={item.count} />
        )
      }
    />
  );
};

const SidebarSearch = () => (
  <div className="flex items-center px-2 pt-2.5">
    <div className="relative w-full">
      <SearchIcon
        className="pointer-events-none absolute top-1/2 left-3 -translate-y-1/2 text-muted-foreground"
        size={14}
      />
      <Input
        aria-label="Search"
        className="h-8 rounded-full border-input-border bg-transparent py-1.5 pr-8 pl-8 text-xs shadow-none focus-visible:bg-background"
        placeholder="Search"
        readOnly
        type="text"
        value=""
      />
    </div>
  </div>
);

// Footer branding row. Mirrors the desktop GatewayMenu: the Closedloop mark, the
// "Closedloop Gateway" label, and the up/down chevron affordance.
const GatewayFooterButton = () => (
  <SidebarMenuButton size="lg" tooltip="Closedloop Gateway">
    <div className="flex size-8 shrink-0 items-center justify-center rounded-lg bg-foreground text-background">
      <ClosedloopMark />
    </div>
    <span className="truncate font-medium">Closedloop Gateway</span>
    <ChevronsUpDownIcon className="ml-auto size-4 opacity-60" />
  </SidebarMenuButton>
);

export const DesktopSidebar = () => (
  <Sidebar variant="inset">
    <MacStoplights />
    <SidebarSearch />
    <SidebarContent className="gap-1 pt-2">
      <SidebarGroup className="p-1">
        <SidebarMenu className="gap-0">
          {mainNav.map((item) => (
            <NavLink item={item} key={item.label} />
          ))}
        </SidebarMenu>
      </SidebarGroup>

      <SidebarCollapsibleSection title="Labs">
        <SidebarMenu className="gap-0">
          {labsNav.map((item) => (
            <NavLink item={item} key={item.label} />
          ))}
        </SidebarMenu>
      </SidebarCollapsibleSection>
    </SidebarContent>

    <SidebarFooter>
      <SidebarMenu>
        <SidebarMenuItem>
          <GatewayFooterButton />
        </SidebarMenuItem>
      </SidebarMenu>
    </SidebarFooter>
  </Sidebar>
);

const ClosedloopMark = () => (
  <svg aria-hidden="true" className="size-4" fill="none" viewBox="0 0 100 100">
    <path
      d="M0.424623 49.6765C0.339767 56.2176 1.55939 62.7103 4.01272 68.7779C6.46604 74.8455 10.1042 80.3673 14.7161 85.0227C19.3281 89.6781 24.8219 93.3744 30.8788 95.8973C36.9358 98.4202 43.4352 99.7193 50 99.7193C56.5648 99.7193 63.0643 98.4202 69.1212 95.8973C75.1782 93.3744 80.672 89.6781 85.2839 85.0227C89.8958 80.3673 93.534 74.8455 95.9873 68.7779C98.4406 62.7103 99.6603 56.2176 99.5754 49.6765C99.5754 49.5115 99.5754 49.3546 99.5754 49.1895H71.7496C71.7496 49.3546 71.7496 49.5115 71.7496 49.6765C71.7496 53.9658 70.473 58.1587 68.0814 61.7249C65.6898 65.2912 62.2906 68.0706 58.3136 69.7117C54.3367 71.3527 49.9606 71.7817 45.739 70.9443C41.5174 70.1069 37.6398 68.0408 34.5966 65.0072C31.5535 61.9737 29.4815 58.109 28.6428 53.902C27.804 49.695 28.2361 45.3346 29.8845 41.3723C31.5329 37.4101 34.3235 34.0239 37.9033 31.6421C41.4831 29.2603 45.6914 27.9899 49.9959 27.9915H50.0704L50.0373 0.280626C43.524 0.275203 37.0735 1.54886 31.0545 4.02881C25.0354 6.50876 19.5659 10.1464 14.9583 14.7338C10.3508 19.3211 6.69575 24.7683 4.20197 30.764C1.70819 36.7597 0.424621 43.1863 0.424623 49.6765Z"
      fill="currentColor"
    />
    <path
      d="M57.1534 0.801147V29.2137C60.1811 30.2616 62.939 31.9629 65.2303 34.1961C67.5215 36.4293 69.2895 39.1392 70.4077 42.1323H99.004C97.3792 31.6897 92.4381 22.0411 84.906 14.6024C77.3738 7.16375 67.6471 2.32669 57.1534 0.801147Z"
      fill="#41A3FF"
    />
  </svg>
);
