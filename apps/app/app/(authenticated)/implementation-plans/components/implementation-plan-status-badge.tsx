...
type ImplementationPlanStatusBadgeProps = {
  status: string;
};
...
const statusStyles: Record<string, string> = {
  ...
  Generating: "bg-purple-100 text-purple-800 border-purple-200 dark:bg-purple-900/30 dark:text-purple-400 dark:border-purple-800",
...
export function ImplementationPlanStatusBadge({ status }: ImplementationPlanStatusBadgeProps) {
  return (
    <Badge
      variant="outline"
      className={cn("font-medium", statusStyles[status] ?? statusStyles.Ready)}
    >
      {status}
    </Badge>
  );
