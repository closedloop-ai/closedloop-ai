    ...
    return (
      <div className="flex items-start gap-2 rounded-md border border-amber-200 bg-amber-50 p-3 dark:border-amber-900 dark:bg-amber-950/20">
        ...
        <p className="text-amber-900 text-sm dark:text-amber-200">
          This issue is not associated with a project. Assign a project to link
          existing PRDs as context.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3 py-2">
      <p className="text-muted-foreground text-sm">
        Link an existing PRD to provide context for this issue.
      </p>
      <Button
        className="w-full"
...
function UploadFileTab({
  ...
  const createAttachment = useCreateContextAttachment(issueId);
...
function ImportGoogleDocsTab({
  ...
  const importMutation = useImportGDriveContext(issueId);
    ...
    <div className="space-y-4 py-2">
      ...
      {!projectId && (
        <div className="flex items-start gap-2 rounded-md border border-amber-200 bg-amber-50 p-3 dark:border-amber-900 dark:bg-amber-950/20">
          <AlertCircleIcon className="mt-0.5 h-4 w-4 flex-shrink-0 text-amber-600 dark:text-amber-500" />
          <p className="text-amber-900 text-sm dark:text-amber-200">
            This issue is not associated with a project. Assign a project to
            import documents.
          </p>
        </div>
      )}
