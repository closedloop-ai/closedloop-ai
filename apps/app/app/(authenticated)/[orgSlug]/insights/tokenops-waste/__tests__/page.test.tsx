...
import TokenOpsWasteRoutePage from "../page";

/**
 * ISS-5280: this screen's per-surface UI gate is retired, so the
 * ONLY route gate left is ISS-5037's Labs container. Mirrors
...
vi.mock("../page-client", () => ({
  TokenOpsWastePageClient: () => <div>TokenOps waste QA target</div>,
}));

describe("tokenops-waste page route", () => {
  // ISS-5037: TokenOps waste is a Labs destination. Leaving this URL reachable
  // while the Labs nav is hidden would defeat the container gate.
  it("renders the screen behind the Labs container route gate", () => {
    render(<TokenOpsWasteRoutePage />);

    expect(screen.getByText("TokenOps waste QA target")).toBeInTheDocument();
    expect(
      screen
        .getByText("TokenOps waste QA target")
  ...
  it("wraps the screen in exactly one route gate", () => {
    const { container } = render(<TokenOpsWasteRoutePage />);
