...
// PLN-529: NewPlanModal now composes `useResolvedJobRepos` and renders
// `JobRepositoriesSection` (which uses `useQueries` for default branches).
// Stub both for structural tests; per-tier resolver behaviour is covered by
// the new-plan-modal-multi-repo and dedicated hook tests.
...
describe("NewPlanModal", () => {
  ...
  describe("Modal with source (PRD mode)", () => {
    ...
    it("should call create+generate mutation with source-derived fields", async () => {
      ...
      // Verify mutation input includes source-derived fields. PLN-602: the
      // legacy `targetRepo`/`targetBranch` fields no longer flow through the
      // create payload — repository identity is server-owned and inherited
      // via `sourceId`.
      ...
      expect(mutationArg.input.targetRepo).toBeUndefined();
      expect(mutationArg.input.targetBranch).toBeUndefined();
    });
    ...
    it("keeps post-create target selection pending when selected-target pre-loop gate blocks", async () => {
