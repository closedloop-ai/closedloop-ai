...
} from "@repo/app/teams/hooks/use-teams";
import {
  useCurrentUser,
...
export function useTeamModal({ team, onSuccess, onClose }: UseTeamModalArgs) {
  ...
  const showRepositoriesTab = isEditMode;
  const repoQueryEnabled =
    isEditMode && !!team?.id && activeTab === TeamModalTab.Repositories;
    ...
    createTeamMutation.mutate(
      { name: name.trim() },
      {
        onSuccess: (newTeam) => {
          ...
          navigation.navigate(`/${orgSlug}/teams/${newTeam.id}/projects`);
          handleClose();
          onSuccess?.();
        },
      }
