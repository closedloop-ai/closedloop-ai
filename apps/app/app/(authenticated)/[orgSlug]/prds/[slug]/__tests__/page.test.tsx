...
// ISS-4526: render-smoke the PRD document-editor page wrapper. This mounts the
// REAL page -> REAL PRDEditorContainer down to its provider-requiring TanStack
// Query hook (`useDocumentBySlug`) under the required QueryClientProvider, and
// asserts the wrapper renders WITHOUT THROWING. This guards the mount-site-
// provider regression class: a shared container that newly calls a
// provider-requiring hook would throw at mount if the provider were absent.
//
...
describe("PrdPage (/prds/[slug]) mount smoke", () => {
  afterEach(() => {
    vi.clearAllMocks();
  });

  it("renders the PRD editor wrapper without throwing under the required providers", async () => {
    const page = await PrdPage(makeParams("checkout-flow"));
  ...
  it("passes a valid version param through to the document fetch", async () => {
    const page = await PrdPage(makeParams("checkout-flow", "2"));
