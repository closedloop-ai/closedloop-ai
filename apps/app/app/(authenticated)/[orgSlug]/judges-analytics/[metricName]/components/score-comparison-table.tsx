...
function getDeltaClassName(delta: number): string {
  ...
  if (delta > JUDGES_ANALYTICS_DELTA_WARNING_THRESHOLD) {
    return "text-warning-foreground font-medium";
  }
  return "";
}
...
function getDocumentHref(row: JudgeScoreRow, orgSlug: string): string {
  const routePrefix = getRoutePrefixForType(row.documentType);
  if (routePrefix === null) {
    return `/${orgSlug}/documents/${row.documentSlug}`;
  }
  return `/${orgSlug}/${routePrefix}/${row.documentSlug}`;
}
            ...
            <TableRow key={row.judgeScoreId}>
              <TableCell>
                <Link
                  ...
                  href={getDocumentHref(row, orgSlug)}
                >
                  {row.documentTitle}
                </Link>
              </TableCell>
