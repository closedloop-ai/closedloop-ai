...
describe("customFieldKeys", () => {
  ...
  test("settings(entityType, entityId) produces unique keys per entity type and id", () => {
    ...
    const keyWorkstream = customFieldKeys.settings(
    ...
    expect(keyProject).not.toEqual(keyWorkstream);
...
describe("useUpdateCustomFieldValue", () => {
  ...
  test("onSuccess invalidates workstreamKeys.detail when entityType is Workstream", async () => {
    mockApiClient.put.mockResolvedValueOnce({});
    ...
    const { result } = renderHook(
      () => useUpdateCustomFieldValue(CustomFieldEntityType.Workstream, "ws-1"),
      { wrapper: createWrapperWithClient(queryClient) }
    );

    result.current.mutate({ fieldId: "field-456", value: null });

    await waitFor(() => expect(result.current.isSuccess).toBe(true));

    expect(invalidateSpy).toHaveBeenCalledWith(
      expect.objectContaining({ queryKey: workstreamKeys.detail("ws-1") })
    );
  });
