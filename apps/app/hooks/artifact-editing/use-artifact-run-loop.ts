 ...
 * ```
 */
export function useArtifactRunLoop({ artifactId }: UseArtifactRunLoopConfig) {
  ...
  const makeRequestChangesHandler = useCallback(
    (command: RunLoopCommand, successMessage: string) =>
      async (changes: string): Promise<boolean> => {
        ...
        try {
          await runLoop.mutateAsync(
            { artifactId, command, prompt: changes },
            {
              onSuccess: () => {
    ...
    [artifactId, prepareConflictRefs, runLoop, routeConflictError]
  );
