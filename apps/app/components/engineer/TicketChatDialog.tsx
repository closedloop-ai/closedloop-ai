...
export function TicketChatDialog({
  ...
  const handleClearChat = async () => {
    if (!confirm("Clear all chat history for this ticket?")) {
      return;
    }
    try {
            ...
            !stream.isStreaming && (
              <div className="flex h-full flex-col items-center justify-center text-center">
                ...
                <p className="mt-1 max-w-[280px] text-muted-foreground/70 text-xs">
                  Get help understanding requirements, scope, or implementation
                  approaches
                </p>
              </div>
            )}
                ...
                return messages.map((msg, idx) => {
                  ...
                  return (
                    <MessageBubble
                      ...
                      key={msg.id}
                      message={msg}
                      onSendAction={isLastAssistant ? sendMessage : undefined}
                    />
                  );
                });
              })()}
