...
/**
 * Phases emitted by the gateway for structured error events.
 * `upsert` — chat identity/binding failed before spawning
 * `spawn`  — provider process failed to start
 * `complete` — post-stream persistence or finalization failed
 */
export type StreamErrorPhase = "upsert" | "spawn" | "complete";

/**
 * Structured error event surfaced to stream consumers. Legacy
 * `{type:"error", error:"..."}` payloads are normalized into this
 * shape with `message` set and other fields undefined.
 */
...
/**
 * Normalize a raw error payload from the stream into a structured
 * `StreamErrorEvent`. Preserves `phase`/`code`/`boundProvider` when
 * present and falls back to the legacy `error` string when `message`
 * is absent.
 */
function toStreamErrorEvent(
  ...
  if (typeof raw !== "object" || raw === null) {
    return { message: fallbackMessage, raw };
  }
  const obj = raw as Record<string, unknown>;
  const rawPhase = obj.phase;
  const phase: StreamErrorPhase | undefined =
    rawPhase === "upsert" || rawPhase === "spawn" || rawPhase === "complete"
      ? rawPhase
      : undefined;
  const code = typeof obj.code === "string" ? obj.code : undefined;
  const boundProvider =
    typeof obj.boundProvider === "string" ? obj.boundProvider : undefined;
  const messageField =
  ...
  const errorField =
    typeof obj.error === "string" && obj.error.length > 0
      ? obj.error
      : undefined;
  const message = messageField ?? errorField ?? fallbackMessage;
  return { phase, code, boundProvider, message, raw };
}
    ...
    try {
      ...
      if (parsed.type === "error") {
        if (parsed.terminal === true) {
          terminalError = true;
          handlers.onError(toStreamErrorEvent(parsed, "Command failed"));
          continue;
        }
