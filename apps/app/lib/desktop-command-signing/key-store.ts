...
/** IndexedDB database version for browser-held Desktop command signing keys. */
export const COMMAND_SIGNING_DB_VERSION = 1 as const;
/** IndexedDB object store name for browser-held Desktop command signing keys. */
...
function openDatabase(): Promise<IDBDatabase> {
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = () => {
      request.result.createObjectStore(COMMAND_SIGNING_STORE_NAME, {
        keyPath: "id",
      });
    };
...
function readStoredKey(db: IDBDatabase): Promise<StoredSigningKey | null> {
  return new Promise((resolve, reject) => {
    const transaction = db.transaction(COMMAND_SIGNING_STORE_NAME, "readonly");
    const request = transaction
      .objectStore(COMMAND_SIGNING_STORE_NAME)
      .get(DEFAULT_COMMAND_SIGNING_KEY_ID);
...
function writeStoredKey(db: IDBDatabase, key: StoredSigningKey): Promise<void> {
  return new Promise((resolve, reject) => {
    const transaction = db.transaction(COMMAND_SIGNING_STORE_NAME, "readwrite");
    const request = transaction
      .objectStore(COMMAND_SIGNING_STORE_NAME)
      .put(key);
...
function deleteStoredKey(db: IDBDatabase): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    const request = transaction
      .objectStore(COMMAND_SIGNING_STORE_NAME)
      .delete(DEFAULT_COMMAND_SIGNING_KEY_ID);
...
async function generateSigningKey(): Promise<StoredSigningKey> {
  ...
  return {
    id: DEFAULT_COMMAND_SIGNING_KEY_ID,
    keyPair,
