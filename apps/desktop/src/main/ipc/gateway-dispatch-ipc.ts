 ...
 * (per AGENTS.md "do NOT reimplement gateway operations") — the loopback pattern
 ...
 *   4. (method)    the channel is GET-only — see step 4 before adding a path.
...
/**
 * EXACT-PATH allowlist (CRITICAL-2). New gateway features add a line here —
 * never a new channel, never a prefix/`startsWith` check (bypassable via `..`,
 * encoded slashes, `//authority`).
 *
 * EMPTY since PLN-1535 M5 deletion 2. Its only entries were the three
 * slug-based read-only PR overlay routes, which are now retired (HTTP 410), so
 * the channel currently permits nothing and every dispatch is refused at step 3.
 * The guard stack is deliberately kept rather than deleted: it is the reviewed
 * transport for renderer→gateway calls, and the next feature that needs one
 * should add a path here instead of reintroducing a channel. Removing the
 * channel, its preload binding, and the routing adapter is a separate
 * security-sensitive change, not a side effect of retiring these routes.
 */
export const GATEWAY_DISPATCH_ALLOWED_PATHS: ReadonlySet<string> =
  new Set<string>();
...
export type GatewayDispatchDeps = {
  ...
  /**
   * Injectable for tests; defaults to `GATEWAY_DISPATCH_ALLOWED_PATHS`.
   *
   * The production set is empty (see above), so without this seam none of the
   * guards below — traversal, authority tricks, header stripping, body caps,
   * log redaction — could be exercised at all: every request would stop at the
   * allowlist check. Those guards are generic and outlive any particular route,
   * so the suite drives them through a synthetic path rather than whichever
   * product route happens to be live this month.
   */
...
export function createGatewayDispatchHandler(deps: GatewayDispatchDeps) {
  ...
  return async (
    ...
    const allowedPaths = deps.allowedPaths ?? GATEWAY_DISPATCH_ALLOWED_PATHS;
    if (!allowedPaths.has(pathname)) {
      return errorEnvelope(403, "path not allowed");
    }

    // 4. GET-only is a constraint on the CHANNEL, not on any particular
    //    route: the allowlist is empty, so nothing reaches here in production.
    //    Whoever adds the first path back must decide deliberately whether it
    //    stays GET-only — otherwise a POST they expect to work is refused 405.
