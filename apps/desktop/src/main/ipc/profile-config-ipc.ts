...
type ProfileConfigIpcDeps = {
  ...
  /**
   * FEA-4005: re-seed repos.json for the newly-applied sandbox scope root, so a
   * profile whose sandbox differs from the previous one takes effect immediately
   * (mirrors the `desktop:update-settings` sandbox-change side effect). Best-effort.
   */
  seedReposConfig: (sandboxBaseDirectory: string) => Promise<void>;
};

/**
 * FEA-4005: re-seed repos for the active sandbox when applying/saving a profile
 * changed the global sandbox scope root. Fire-and-forget; seeding is a setup
 * convenience and must never block the config apply/save.
 */
function reseedReposOnSandboxChange(
  deps: ProfileConfigIpcDeps,
  previousSandbox: string
): void {
  const currentSandbox = deps.settingsStore.getSandboxBaseDirectory();
  if (currentSandbox && currentSandbox !== previousSandbox) {
    deps.seedReposConfig(currentSandbox).catch(() => {
      // seedReposConfig is already best-effort; swallow to keep apply non-blocking.
    });
  }
}
...
function getConfigConnectionPatch(
  ...
  return {
    ...
    // FEA-4005: thread the per-profile sandbox through unchanged when present.
    // Require a string — coercing a non-string via `String()` would turn null,
    // a number, or an object into relative path text that `normalizeScopePath`
    // then resolves under the process cwd into a valid persisted sandbox.
    // Malformed IPC values are dropped (omission preserved); normalization +
    // FEA-3641 risky-root rejection happen in the store's
    // `validateSandboxBaseDirectory` for real string paths.
    ...(typeof payload.sandboxBaseDirectory === "string"
      ? { sandboxBaseDirectory: payload.sandboxBaseDirectory }
      : {}),
    ...(snapshot ?? {}),
  };
}
...
export function registerProfileConfigIpcHandlers(
  ...
  ipcMainLike.handle(
    ...
    (event, rawPayload) => {
      ...
      const previousSandbox = deps.settingsStore.getSandboxBaseDirectory();
      ...
      if (profileId) {
        ...
        if (isActiveProfile) {
          // refreshActiveProfileAfterSave re-applies the config, pushing this
          // profile's sandbox to the global scope root; re-seed if it changed.
          refreshActiveProfileAfterSave(deps, savedConfig, explicitApiKey);
          reseedReposOnSandboxChange(deps, previousSandbox);
        }
        return savedConfig;
      }
