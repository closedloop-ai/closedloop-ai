...
import { getOtlpReceiverState } from "../telemetry/otlp-receiver-state.js";
...
import { isClaudeLiveHookEnabled } from "./claude-live-hook-flag.js";
...
const lifecycle = createAgentMonitorHooksLifecycle({
  ...
  // FEA-3729: gate the Claude live hook path off (keeps the code, flips the
  // collection mode to watcher, self-heals installed entries on boot).
  isHookPathEnabled: isClaudeLiveHookEnabled,
  installHooks,
  uninstallHooks,
