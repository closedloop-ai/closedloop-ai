...
} from "../telemetry/codex-otel-config-core.js";
...
export type AgentMonitorHooksLifecycleInput = {
  ...
  /**
   * FEA-3729 kill switch. When it returns `false` the Claude live hook path is
   * gated OFF: the master toggle reports disabled and enabling is inert, while
   * boot self-heals any previously-installed entries by uninstalling them.
   * Defaults to always-on so existing callers and tests are unchanged. The
   * persisted toggle intent (`getEnabled`) is left untouched, so flipping the
   * switch back on restores whatever the user had configured.
   */
...
export function createAgentMonitorHooksLifecycle(
  ...
  const isHookPathEnabled = input.isHookPathEnabled ?? (() => true);
  // FEA-3729: the master toggle only reports enabled when BOTH the kill switch
  // is on and the user opted in. This is the value `currentHooksState().claude`
  // reads, so a killed hook resolves the collection mode to `"watcher"`.
  const isAgentMonitorHooksEnabled = () =>
    isHookPathEnabled() && input.getEnabled() === true;

  return {
    isAgentMonitorHooksEnabled,
    setAgentMonitorHooksEnabled: (enabled) => {
      // FEA-3729: kill switch off → the toggle is inert. Report disabled without
      // installing or mutating the persisted intent; boot owns the uninstall.
      if (!isHookPathEnabled()) {
    ...
    syncAgentMonitorHooksOnBoot: () => {
      // FEA-3729: kill switch off → self-heal by removing any hook entries a
      // prior build wrote (the persisted flag may still be `true`), so stale
      // hook commands stop POSTing. Leave the persisted intent untouched.
      if (!isHookPathEnabled()) {
        try {
          logLifecycleWarnings(input.uninstallHooks(), input.onWarning);
        } catch (error) {
          input.onBootRepairError(
            `boot hook uninstall failed: ${
