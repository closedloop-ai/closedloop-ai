...
import { asRecord, stringField } from "../util/api-response-utils.js";
 ...
 * transport (PoP signing, fetch, status→error mapping) is identical, so those
 * clients reuse this instead of reimplementing it.
 */
export async function postSignedDesktopSession<T>(
  ...
  if (!response.ok) {
    return {
      ok: false,
      ...mapDesktopSessionErrorResponse(response.status, parsedBody),
    };
  }
...
/**
 * Map an HTTP status + desktop-contract error body to the typed session error.
 * Shared across the PoP-signed desktop-session endpoints so the status→error
 * mapping stays identical.
 */
export function mapDesktopSessionErrorResponse(
  status: number,
  body: unknown
