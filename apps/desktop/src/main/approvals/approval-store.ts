...
export type ApprovalStoreOptions = {
  ...
  /**
   * Override the electron-store directory. Production leaves this unset (default
   * userData path); tests pass a temp dir so each store gets an isolated file
   * instead of sharing the single hardcoded "desktop-approvals" store.
   */
  cwd?: string;
};
