import { HEALTH_CHECK_REPAIR_OPERATION_ID } from "@repo/api/src/types/compute-target";
...
> = {
  health_check: "low",
  // Repair writes to this machine, and both halves are bounded: it clears ONLY
  // binary-path overrides it just proved stale (a working override is never
  // touched), and runs `claude plugin enable` for Closedloop plugins already
  // installed-but-disabled. It installs nothing and deletes nothing, and the
  // sweep it triggers runs with plugin auto-remediation requested, so an enable
  // is part of its blast radius by design. Both actions are no-ops on a second
  // press. That is the same tier as `binary_paths_settings`, which is the
  // narrower half of what it does.
  [HEALTH_CHECK_REPAIR_OPERATION_ID]: "medium",
  repos_config: "medium",
  filesystem: "medium",
  symphony_status: "low",
  ...
  // A member-dispatched pack install runs a vetted installer that modifies the
  // node's environment — treat it as high-risk, on par with deploy.
  member_pack_install: "high",
};
