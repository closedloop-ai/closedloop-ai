...
export function resolveOperationId(pathname: string): OperationId | null {
  ...
  if (pathname === MEMBER_PACK_INSTALL_PATH) {
    return MEMBER_PACK_INSTALL_OPERATION_ID;
  }
  if (
  ...
  // Repair MUTATES the machine, so it carries its own operation id and risk
  // tier rather than riding on read-only `health_check` (ISS-5389 review).
  if (pathname === HEALTH_CHECK_REPAIR_PATH) {
    return HEALTH_CHECK_REPAIR_OPERATION_ID;
  }
