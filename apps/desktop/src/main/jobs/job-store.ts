...
// LocalJobStatus / LocalJobCommand are defined once in the renderer-safe
// contract module (activity-panel-contract.ts) and re-exported here so existing
// main/server importers keep their `job-store.js` import path.
export type {
  LocalJobCommand,
  LocalJobStatus,
} from "../../shared/activity-panel-contract.js";

import type { UserVisibleLoopFailurePayload } from "../loop/user-visible-loop-failure.js";
...
export type LocalJob = {
  ...
  /**
   * ISS-5872 — required artifacts a would-be-COMPLETED run never wrote, which
   * is why it was downgraded to FAILED. Present ONLY on that downgrade, so it
   * never relabels a job that failed for its own reason. Absent on jobs
   * persisted by older builds; consumers treat absence as "nothing missing".
   */
...
// Compile-time guard: LocalJob must remain a structural superset of the
// renderer-facing ActivityJob contract, so the ActivityPanel can render a
// LocalJob (completed-jobs payload) through the shared type. If a field the
// renderer relies on is renamed or narrowed here, this fails to compile.
export type _LocalJobSatisfiesActivityJob = AssertAssignable<
  LocalJob,
  ActivityJob
>;
