...
} from "../loop/loop-finalizer.js";
import { refreshLoopTokenSingleflight } from "../loop/loop-refresh.js";
import { LoopSchedulerContext } from "../loop/loop-scheduler-context.js";
import {
  type ClassifierProvenanceContext,
  classifyLoopStatus,
  type TerminalReason,
} from "../loop/loop-status-classifier.js";
import type { LoopTokenStore } from "../loop/loop-token-store.js";
import type { TelemetryEmitter } from "../telemetry/telemetry-protocol.js";
...
export type BootRecoveryDeps = {
   ...
   */
  getApiKeyProvenance?: () =>
    | import("../settings/api-key-store.js").ApiKeyProvenance
    | null;
...
export class BootRecoveryService implements Disposable {
  ...
  private async reattachLiveJob(
    ...
    if (!isProcessRunning(pid)) {
      // Drop the boot seed (`seedRunningLoopsFromJobStore`): its runner is gone,
      // and a stale entry would 409 a later legitimate launch of this loopId.
    ...
    if (disposition.kind === "terminal") {
      ...
      // and skip reattach.
      //
      // Also drop the boot seed (`seedRunningLoopsFromJobStore`), which
      // registered this loopId before the cloud verdict was known. Leaving it
      // would 409 every later launch of this loopId for the life of the
      // process -- the seed exists to guard a REPLAY window, not to outlive a
      // loop the cloud has already finalized.
      unregisterLoop(loopId);
