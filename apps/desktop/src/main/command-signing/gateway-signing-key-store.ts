...
/**
 * Reasons that mean the persisted key is permanently unusable, so
 * {@link GatewaySigningKeyStore.getOrCreate} should drop it and regenerate
 * rather than surface a dead end:
 * - `key_missing`: no material stored yet (the original create path).
 * - `decrypt_failed`: ciphertext exists but `safeStorage` can no longer decrypt
 *   it — the OS encryption context changed (keychain reset after an OS update,
 *   "Login" keychain reset, or the profile moved to another machine/user). The
 *   bytes can never be recovered, so the only path forward is a fresh key.
 * - `key_import_failed`: decryption succeeded but the plaintext is not a valid
 *   private key (corrupt material) — equally unusable.
 * Deliberately excluded: `safe_storage_unavailable` (transient/environmental —
 * regenerating would churn keys) and `gateway_id_missing` (a caller error).
 */
const REGENERABLE_REASONS: ReadonlySet<GatewaySigningKeyUnavailableReason> =
  new Set(["key_missing", "decrypt_failed", "key_import_failed"]);
...
export class GatewaySigningKeyStore {
  ...
  /**
   * Loads the existing keypair for a gatewayId, or generates one when there is
   * no usable stored key. "No usable key" covers both an absent key and stored
   * material that can never be used again (see {@link REGENERABLE_REASONS}) —
   * notably a `decrypt_failed` key after the OS encryption context changed, which
   * previously bricked every consumer (e.g. browser sign-in reported a generic
   * "check your connection" error with no recovery). Present-but-unusable
   * material is dropped before regenerating so the corrupt ciphertext cannot
   * linger if regeneration itself fails. Environmental/transient failures
   * (`safe_storage_unavailable`) and caller errors (`gateway_id_missing`) are
   * returned unchanged.
   */
  getOrCreate(gatewayId: string): GatewaySigningKeyResult {
    const existing = this.load(gatewayId);
    if (existing.ok || !REGENERABLE_REASONS.has(existing.reason)) {
      return existing;
    }
    if (existing.reason !== "key_missing") {
      // Undecryptable/corrupt material exists on disk; remove it so a failed
      // regeneration can't leave the stale key to fail identically next time.
