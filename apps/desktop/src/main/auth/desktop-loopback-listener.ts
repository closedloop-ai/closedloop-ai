...
/** The query params delivered to the callback path (any may be absent). */
export type LoopbackCallback = {
  code: string | null;
  state: string | null;
  /**
   * OAuth `error` code when the web consent screen ENDED the run rather than
   * approving it — today only `access_denied`, sent when the user cancels.
   *
   * Without this a cancel is indistinguishable from silence, and the desktop
   * waits out its full sign-in timeout while the browser tab has already said
   * "return to the desktop app". Optional-by-absence on purpose: an older web
   * app that never sends it still produces exactly the callback shape this
   * listener has always produced.
   */
  error: string | null;
};
