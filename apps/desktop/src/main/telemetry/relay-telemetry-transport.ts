...
export type DesktopTelemetryTransport = {
  ...
  /**
   * Bounded-drain pending exports — both in-flight sends and anything still
   * queued for a session — WITHOUT disconnecting, so a crash handler can get
   * the exception it just emitted onto the wire and then continue (ISS-6328).
   * Shares `stop`'s time cap, but leaves the transport usable — which matters
   * because the non-Error `unhandledRejection` path emits and then lets the app
   * keep running.
   */
  flush: () => Promise<void>;
...
class RelayTelemetryTransport implements DesktopTelemetryTransport {
  ...
  /** In-flight send chains, awaited (time-capped) by stop() and flush(). */
  ...
  /** Resolvers waiting for the warm-up queue to be handed to a live session. */
  private readonly warmUpFlushWaiters: Array<() => void> = [];
  ...
  async stop(): Promise<void> {
    ...
    this.releaseWarmUpFlushWaiters();
    await this.drainPendingExports();
    this.session = null;
    this.handshakeInFlight = null;
    this.context = null;
    this.disconnectSocket();
  ...
  async flush(): Promise<void> {
    // Deliberately does NOT set `stopped` or tear the socket down: the caller
    // may still be running (the non-Error unhandledRejection path), and a
    // crash caller exits on its own terms right after.
    await this.drainPendingExports();
  }

  export(signal: RelayTelemetrySignal, body: Uint8Array): Promise<boolean> {
    // Track the whole export chain so the bounded drains can wait on it. Added
    // synchronously here (before the first await) so a send started by the SDK's
    // final shutdown flush is visible to a stop()/flush() that runs right after.
    return this.track(this.runExport(signal, body));
  }
  ...
  private async performHandshake(
    ...
    // Typed as the contract, not `Record<string, string>`: `emitWithAck` takes
    // `payload: unknown`, so the keys-covered guard on the schema constrains
    // only the PARSER. Without this annotation a sender-only field still
    // compiles here and the `.strict()` relay parser then rejects the whole
    // handshake at runtime — the same silent-loss class the guard exists to
    // close, just moved to the producer side.
    const request: KeylessTelemetrySessionRequest = {
  ...
  private async sendNow(
    ...
    // Same reason as the handshake request above: annotate the envelope so the
    // producer is bound to the contract the relay's `.strict()` schema parses.
    const envelope: KeylessTelemetryEnvelope = {
  ...
  private flushWarmUpQueue(session: ActiveSession): void {
    ...
    for (const entry of pending) {
      // Fire-and-forget but TRACKED: a warm-up send issued while a crash-path
      // flush() is draining is the send that carries the exception, so the
      // drain has to be able to wait on it. sendNow CAN re-enqueue an entry if
      // the socket drops mid-flush, but that cannot loop unboundedly:
      // re-enqueue is bounded by warmUpQueueLimit (oldest-dropped), and the
      // only send retry path (invalid_session) is guarded by allowRehandshake.
      this.track(this.sendNow(entry.signal, entry.body, session)).catch(
        () => undefined
      );
    }
    this.releaseWarmUpFlushWaiters();
  ...
  /**
   * Bounded drain shared by stop() and flush(): hand anything parked in the
   * warm-up queue to a live session, then let every tracked send settle, all
   * inside one `shutdownDrainMs` window.
   *
   * Waiting only on `inFlightExports` is not enough on the crash path. A crash
   * before the first relay session — or during a reconnect — leaves the
   * exception in `warmUpQueue` with its export() chain already resolved false
   * and removed, so the drain would see an empty set and the process would exit
   * with the exception only in memory (ISS-6328).
   */
  private async drainPendingExports(): Promise<void> {
    if (this.warmUpQueue.length === 0 && this.inFlightExports.size === 0) {
      return;
    }
    // Whichever finishes first: everything settles, or the grace elapses.
    await Promise.race([
      this.drainQueuedThenInFlight(),
      this.delay(this.shutdownDrainMs),
    ]);
  }

  private async drainQueuedThenInFlight(): Promise<void> {
    try {
      if (this.warmUpQueue.length > 0) {
        // Kicks a handshake when the socket is up but sessionless (a refused or
        // expired session). When it is not up yet, the socket's own `connect`
        // handler runs the handshake and flushWarmUpQueue releases the wait.
        await this.ensureSession();
        await this.waitForWarmUpFlush();
      }
      await this.settleInFlightExports();
    } catch (error) {
      this.log.debug(`keyless telemetry drain error: ${describeError(error)}`);
    }
  }

  private waitForWarmUpFlush(): Promise<void> {
    if (this.warmUpQueue.length === 0) {
      return Promise.resolve();
    }
    return new Promise<void>((resolve) => {
      this.warmUpFlushWaiters.push(resolve);
    });
  }

  private releaseWarmUpFlushWaiters(): void {
    const waiters = this.warmUpFlushWaiters.splice(
      0,
      this.warmUpFlushWaiters.length
    );
  ...
  private settleInFlightExports(): Promise<void> {
    if (this.inFlightExports.size === 0) {
      return Promise.resolve();
    }
    return Promise.allSettled([...this.inFlightExports]).then(() => undefined);
  }

  /** Register an export chain so the bounded drains can wait on it. */
  private track(pending: Promise<boolean>): Promise<boolean> {
    this.inFlightExports.add(pending);
    pending
      .finally(() => this.inFlightExports.delete(pending))
      .catch(() => {
        // finally() re-raises the settlement, so this keeps a rejected send
        // from surfacing as an unhandled rejection. Callers handle the returned
        // promise themselves; allSettled tolerates a rejection either way.
      });
    return pending;
  }
