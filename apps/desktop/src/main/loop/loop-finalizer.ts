...
} from "../cost/token-usage.js";
import { ReconciliationStatus } from "../cost/usage-reconciliation.js";
import {
  buildUsageReconciliation,
  toCompletedEventUsageReconciliation,
} from "../cost/usage-reconciliation-event.js";
import {
  readLogTail,
  readTextFile,
  sanitizeErrorMessage,
} from "../diagnostics/diagnostics-helpers.js";
import {
  isTerminalJobStatus,
  type JobStore,
  type LocalJob,
...
import { ingestHarnessSessionLimits } from "../session-limits/harness-ingest.js";
...
import {
  ...
  resolveJobFailureReason,
} from "./loop-failure-reason.js";
...
import { downgradeJobForMissingArtifacts } from "./missing-required-artifacts.js";
import { readArtifacts } from "./read-loop-artifacts.js";
...
export async function tryPostCompletedEvent(
  ...
  // PRD-538: capture Claude Code's authoritative result envelope (when present)
  // and reconcile our derived cost against it. Additive event fields — safe for
  // an API that has not yet learned to persist them.
  const usageReconciliation = buildUsageReconciliation(
    parseHarnessResult(claudeWorkDir),
    tokensUsed.tokensByModel
  );

  // FEA-3523: ride the same stdout seam to feed the COARSE `rate_limit_event`
  // source into the session-limit snapshot store, lighting up the renderer's
  // subscription-limit bars with no opt-in. Fail-closed — never breaks finalize.
  ingestHarnessSessionLimits(claudeWorkDir);

  if (
    usageReconciliation.reconciliation.status === ReconciliationStatus.Drifted
  ) {
    gatewayLog.warn(
      "loop-finalizer",
      `loopId=${job.loopId} cost reconciliation DRIFT: derived=${usageReconciliation.derivedCostUsd} authoritative=${usageReconciliation.authoritativeCostUsd} deltaUsd=${usageReconciliation.reconciliation.deltaUsd}`
    );
  }
  ...
  const completedEvent: Record<string, unknown> = {
    ...
    usageReconciliation:
      toCompletedEventUsageReconciliation(usageReconciliation),
...
/**
 * Classify a non-success terminal job into the code + message the cloud error
 * event carries. Precedence, most specific first:
 *
 * 1. a trusted `loop-error.json` marker the runner signed;
 * 2. a 0-token EXECUTE (ghost loop);
 * 3. ISS-5872 — a run downgraded from COMPLETED because its required artifact
 *    was never written, which names the exact files so the operator does not
 *    have to open the run to learn what is missing;
 * 4. the generic process-level outcome.
 *
 * The error is never null on a non-success terminal state: every branch returns
...
export async function tryPostErrorEvent(
  ...
  // FEA-3523: rate-limited exits route FAILED/STOPPED/UNKNOWN jobs through this
  // error path, not `tryPostCompletedEvent`. Ingest the COARSE `rate_limit_event`
  // here too so a rejected run — the scenario that most needs the coarse source —
  // still lights up the renderer's subscription-limit bars. The completed and
  // error paths are mutually exclusive (see finalize's if/else-if), so this never
  // double-records. Fail-closed — never breaks finalize.
  ingestHarnessSessionLimits(claudeWorkDir);
  ...
  const { code: errorCode, message: errorMessage } = resolveJobFailureReason(
    job,
    userVisibleFailure
  );
  ...
  const errorEvent: Record<string, unknown> = {
    ...
    ...(hasTokenActivity
      ...
      : {}),
    // Per-model attribution, so the cloud does not price a real failed run
    // against a synthetic default model. `LoopEventErrorSchema.tokensByModel`
    // already accepts it; it stays OMITTED (never null) when the run reported
    // no per-model usage, so a cloud build predating this sees today's payload.
    ...(Object.keys(tokenUsage.tokensByModel).length > 0
...
export function persistFinalJobStatus(
  ...
  jobStore.upsert({
    ...
    // ISS-5872 — carried explicitly, like liveActivity above: `current` is the
    // store's copy, which predates the downgrade decision, so spreading it
    // alone would drop the reason this job is FAILED and leave a later
    // recovery pass unable to say what was missing.
    missingRequiredArtifacts:
      job.missingRequiredArtifacts ?? current.missingRequiredArtifacts,
    updatedAt: now,
...
export function emitFinalizationTelemetry(
  ...
  // OUTCOME decides severity and message; `reason` only decides wording, and
  // never overrides the outcome (ISS-5872 review). Reading "live-exit" as
  // "succeeded" was safe while the live path could only reach here on success,
  // but a run downgraded for a missing deliverable is FAILED on live-exit, and
  // reporting that as `info` / "Job completed successfully" would tell
  // telemetry the exact opposite of what the cloud was told.
  const telemetrySeverity =
    isSuccessStatus || job.status === "CANCELLED" ? "info" : "error";
...
export async function finalizeLoopFromRuntime(
  ...
  // ISS-5872 — a run may not claim COMPLETED while the artifact it exists to
  // produce is missing. Runs after the 0-token guard above so a ghost EXECUTE
  // keeps its more specific NO_WORK_PRODUCED classification.
  resolvedJob = downgradeJobForMissingArtifacts(resolvedJob, {
    claudeWorkDir,
    worktreeDir,
    reason,
  });
  ...
  const now = new Date().toISOString();
  // ISS-5872 — a downgrade must reach disk before the first cloud await, not
  // only after it. The live-exit path normally persists its final status last,
  // so a crash between `cloudFinalizedAt` being stored and that persist would
  // leave the job locally RUNNING with a cloud marker: boot reconciliation
  // promotes a dead RUNNING job, and recovery then skips it because the marker
  // exists, so the desktop would show COMPLETED for a run the cloud was told
  // had FAILED. Persisting here closes that window; `persistFinalJobStatus` is
  // a no-op once `finalStatusPersistedAt` is set, so the tail call below stays
  // correct for every other status.
  const persistBeforeCloud =
    reason !== "live-exit" ||
    resolvedJob.missingRequiredArtifacts !== undefined;
  ...
  if (isSuccessStatus) {
    ...
    // ISS-5872 — a run downgraded for a missing deliverable still wrote real
    // work (the live incident left investigation notes and open questions), and
    // the success branch above is the only other caller of tryUploadArtifacts.
    // Upload the partials here so the operator keeps them, matching the legacy
    // path where the upload had already run before the terminal decision.
    let partialUploadError: string | undefined;
    if (resolvedJob.missingRequiredArtifacts) {
      const partialUpload = await tryUploadArtifacts(
        resolvedJob,
        command,
        claudeWorkDir,
        worktreeDir,
      ...
      if (partialUpload.failed) {
        partialUploadError = partialUpload.error ?? "Partial upload failed";
        gatewayLog.warn(
          "loop-finalizer",
          `Partial artifact upload failed for ${command} loopId=${resolvedJob.loopId}: ${partialUpload.error}`
        );
      }
    }
    ...
    if (postResult.failed) {
      ...
      partialUploadError &&
      isRetryableFinalizationError(partialUploadError)
    ) {
      // ISS-5872 — the terminal error reached the cloud but the partials did
      // not. Marking the job cloud-finalized here would tear the loop down and
      // delete its token, and recovery skips a job that carries the marker, so
      // the investigation output the operator was downgraded FOR would be gone
      // for good. Leave it unfinalized instead — exactly what the success
      // branch above already does when its upload fails. The replay is safe in
      // both directions: `artifactsUploadedAt` is stamped only on a successful
      // upload, so the retry re-attempts it, while `completedEventPostedAt`
      // makes `tryPostErrorEvent` a no-op, so the terminal event is not sent
      // twice.
      remoteError = partialUploadError;
