...
function dotContent(index: number, state: StepState) {
  ...
  if (state === "error") {
    return <TriangleAlertIcon aria-hidden="true" className="size-4" />;
  }
  return (
    <span
      className={cn(
        "flex size-full items-center justify-center rounded-full",
        // The active pulse runs on the compositor (data-ob-motion), so it keeps
        // moving even while the main thread is blocked during the import.
...
/**
 * Scan → Import → Compute → Ready progress stepper. Short labels (FEA-4057) so
 * they stay legible at narrow desktop widths rather than truncating.
 */
