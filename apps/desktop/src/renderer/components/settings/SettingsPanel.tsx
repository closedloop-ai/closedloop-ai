  ...
  return (
    <div className="mt-4 space-y-4">
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center justify-between">
          <div>
            <p className="font-medium text-sm">Claude Code Session Tracking</p>
            <p className="text-[var(--muted-foreground)] text-xs">
...
function PoliciesTab({
  ...
  const rules =
    (settings?.alwaysAllowRules as Array<{
      id: string;
      description?: string;
      command?: string;
    }>) ?? [];
        ...
        <CardContent>
          {rules.length === 0 ? (
            <p className="py-4 text-center text-[var(--muted-foreground)] text-sm">
              No always-allow rules configured
            </p>
          ) : (
              ...
              {rules.map((rule) => (
                ...
                >
                  <div className="min-w-0 flex-1">
                    ...
                    {rule.command && (
                      <p className="truncate font-mono text-[var(--muted-foreground)] text-xs">
                        {rule.command}
                      </p>
                    )}
                  </div>
                  <Button
                    className="ml-2 shrink-0 text-[var(--destructive)]"
...
const LAB_FLAGS: {
  ...
  {
    key: "agentMonitorEnabled",
    label: "Agent Dashboard",
    description: "Enable the local SQLite-backed Agent Dashboard.",
    category: "Monitoring",
    requiresRestart: true,
  },
  {
    key: "planExtractionEnabled",
    label: "Plan Extraction",
    description: "Enable Plans / plan extraction UI in the Agent Dashboard.",
    category: "Monitoring",
  },
  {
    ...
    description:
      "Show personalized agentic-development coaching in the Sessions view.",
    category: "Experimental",
  },
  {
    key: "commandSigningEnforcementEnabled",
    label: "Command Signing Enforcement",
    description: "Require ED25519 signatures on browser commands.",
    category: "Security",
    requiresRestart: true,
  },
  {
          ...
          {selectedProfile && (
            <div className="mt-4 space-y-3 border-t pt-4">
              ...
              <div className="flex justify-end">
                <Button
                  disabled={
                    profileSaving || !isProfileFormComplete(selectedForm)
                  }
                  onClick={() => {
