/**
 * @file claude-token-dedup.test.ts
...
test("Claude parser drops an unsafe token snapshot and keeps parsing (no abort)", async () => {
  // A single assistant turn whose `cache_read` counter exceeds the JS-safe
  // storage contract (`InvalidTokenCountError`). Before FEA-2717's graceful
  // degradation this aborted the whole transcript; now that ONE event's usage is
  // dropped and the message text/model still render — a small token-total gap,
  ...
  assert.ok(texts.includes("response text"));
  assert.ok(texts.includes("safe response"));
  // Only the good event's usage is counted; the bad snapshot is dropped
  // (not rounded, not zeroed, not aborted).
  assert.deepEqual(parsed.tokensByModel["claude-opus-4-5"], {
    input: 20,
    output: 7,
    cacheRead: 3,
...
test("Claude dedup: <synthetic> model lines are ignored", async () => {
  const filePath = writeClaudeTranscript("dedup-synthetic", [
    ...
    // The positive control: a genuine turn the parser MUST count, so the
    // assertion below distinguishes "skipped `<synthetic>`" from "parsed nothing".
    {
      type: "assistant",
      timestamp: "2026-06-07T10:00:10.000Z",
      uuid: "u2",
  ...
  // No tokens attributed to the synthetic line — and the real-model turn beside
  // it IS attributed. Without that second turn this asserts only an absence, so
  // a parser that read nothing at all would satisfy it just as well as one that
  // correctly skipped `<synthetic>`.
  assert.deepEqual(parsed.tokensByModel, {
    "claude-opus-4-5": { input: 100, output: 50, cacheRead: 0, cacheWrite: 0 },
  });
  assert.equal(parsed.tokenSeries.length, 1);
  assert.equal(parsed.tokenSeries[0].model, "claude-opus-4-5");
});
