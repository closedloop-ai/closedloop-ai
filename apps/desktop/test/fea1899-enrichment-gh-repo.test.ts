/**
 * @file fea1899-enrichment-gh-repo.test.ts
 ...
 * path end-to-end.
 *
 * FEA-1791 Phase 3: repo-identity + historical-backfill moved off the raw
 * `SqliteExecutor` onto the single `DesktopPrisma` client — typed delegates for
 * the reads (`resolveRepoForCwd`/`resolveRepoByFullName`/`isKnownRepoPath`) and
 * raw `$queryRawUnsafe`/`$executeRawUnsafe` (inside `prisma.write`) only for the
...
// FEA-1791 Phase 3: repo-identity moved off the raw `SqliteExecutor` onto the
// single `DesktopPrisma` client, so this harness uses the shared `openTestPrisma`
// — which builds the client over a migrated libSQL file with the PRODUCTION
// `createWriteQueue` (electron-free, so it runs locally AND in the node-test job).
// The functions under test read/write through `db.prisma`; seeding/assertion SQL
// uses the raw `db.store` handle on the SAME file (WAL → cross-connection
