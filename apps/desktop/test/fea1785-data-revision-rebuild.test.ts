/**
 * @file fea1785-data-revision-rebuild.test.ts
...
import { runDataRevisionRebuild } from "../src/main/collectors/data-revision-rebuild.js";
import { HistoricalParseWorkerFailureError } from "../src/main/collectors/historical-parse-worker-protocol.js";
import type {
  Harness,
  HarnessCollector,
  NormalizedSession,
...
/** Create a minimal fake collector for orchestrator tests. */
function fakeCollector(
  ...
  return {
    ...
    parse: opts.parse ?? (() => Promise.resolve([])),
...
test("3: Rebuild correctness (AC1) — rebuilt session matches fresh-import values", async () => {
  ...
  try {
    ...
    const preEvents = await db.prisma.client.$queryRawUnsafe<{ cnt: number }[]>(
    ...
    const staleLink = await db.prisma.client.$queryRawUnsafe<{ cnt: number }[]>(
...
test("4e: Data-revision rebuild counts parser-output failures as parseErrors without missingSource", async () => {
  ...
  assert.equal(
    logs.some((message) => message.includes("parseError=parser-output-error")),
    true
  );
  assert.equal(
...
test("4e.1: Unmapped parser-output failure does not clear later stale source rebuilds", async () => {
  ...
  const result = await runDataRevisionRebuild({
    ...
    parseSource: (_parsedCollector, source) => {
      ...
      return Promise.resolve([makeSession({ sessionId: "copilot-chat-good" })]);
    },
  });
...
test("4f: Data-revision rebuild keeps ordinary read failures retryable as missingSource", async () => {
  ...
  const result = await runDataRevisionRebuild({
    ...
    parseSource: () => Promise.reject(new Error("file changed while reading")),
  });
...
test("7: Wipe-and-reimport equivalence (AC5) — rebuilt DB aggregates match fresh-import DB", async () => {
  ...
  const session = makeSession({
    ...
    messageTimestamps: ["2026-06-07T10:00:30.000Z", "2026-06-07T10:01:00.000Z"],
    toolUses: [
...
test("12: Orphan-free deletion — deleteSessionRow removes rows from all child tables + sessions", async () => {
  ...
  try {
    ...
    for (const t of tables) {
      const result = await db.prisma.client.$queryRawUnsafe<{ cnt: number }[]>(
        `SELECT COUNT(*) AS cnt FROM ${t.name} WHERE ${t.col} = $1`,
        sid
      );
      assert.ok(result[0].cnt > 0, `${t.name} should have rows before delete`);
    }
...
test("13: Concurrent import during rebuild — both sessions complete correctly via write-queue serialization", async () => {
  ...
  try {
    ...
    const [rebuildResult] = await Promise.all([rebuildPromise, importPromise]);
...
test("15: Reactivated rebuild — main agent recreated as waiting, not completed", async () => {
  ...
  try {
    ...
    const preRow = await db.prisma.client.$queryRawUnsafe<{ status: string }[]>(
...
// ═══════════════════════════════════════════════════════════════════════════
// TEST 15: Metadata refresh — rebuild updates sessions.metadata
// ═══════════════════════════════════════════════════════════════════════════

test("15: Metadata refresh — rebuild replaces stale metadata with current parse output", async () => {
  const dir = await mkdtemp(path.join(os.tmpdir(), "fea1785-t15-"));
  const db = await openTestDb(dir);
  try {
    ...
    assert.equal(metaAfter.userMessages, 12, "metadata reflects current parse");
    assert.equal(metaAfter.plans.length, 2, "plans updated from current parse");
  } finally {
    await db.close();
    await rm(dir, { recursive: true, force: true });
  }
...
test("16: Rebuild bumps updated_at so revision-gated cloud replace triggers", async () => {
  ...
  try {
    const sid = "sync-gate";
    await db.importer.importSession(makeSession({ sessionId: sid }), "claude");
