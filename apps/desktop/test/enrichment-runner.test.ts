/**
 * @file enrichment-runner.test.ts
 ...
 *
 * The runner runs on the single `DesktopPrisma` client — typed delegates for the
 * eligible reads/writes (`findMany`/`findFirst`, `updateMany` for the lease CAS,
 * attempt increment, apply, mark-not-applicable, and the link `upsert`) with raw
 * `$executeRawUnsafe` kept only for the two COALESCE-preserve /
 * conditional-ON-CONFLICT writes that have no typed form. This suite is the
 * contract guard: the harness
...
// The runner takes the single `DesktopPrisma` client, so this harness uses the
// shared `openTestPrisma` — which builds the client over a migrated libSQL file
// with the PRODUCTION `createWriteQueue` (electron-free, so it runs in the normal
// node-test job AND locally) — exercising the runner end-to-end: the runner
// reads/writes through the Prisma client, and the seeding/assertion SQL (`q`)
// uses the raw store handle on the SAME file (WAL gives cross-connection
