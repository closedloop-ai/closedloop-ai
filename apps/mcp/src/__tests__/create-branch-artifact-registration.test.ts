...
const TOOL_NAME = "create_branch_artifact";
 ...
 * inversion or a dropped `requiresWrite` flag fails the test. index.js reads
...
describe("create_branch_artifact tool registration gating", () => {
  ...
  it("registers create_branch_artifact only when the session has write scope", async () => {
    const withWrite = await registeredToolNames(["read", "write"]);
    expect(withWrite).toContain(TOOL_NAME);
  });

  it("withholds create_branch_artifact from read-only sessions", async () => {
