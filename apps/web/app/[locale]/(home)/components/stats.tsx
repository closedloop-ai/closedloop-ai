...
type StatsProps = {
  cmsData?: HomePage["stats"] | null;
  dictionary: Dictionary;
};

export const Stats = ({ cmsData, dictionary }: StatsProps) => {
  // Use CMS data if available, otherwise fall back to dictionary
  const title = cmsData?.title ?? dictionary.web.home.stats.title;
  const description =
    cmsData?.description ?? dictionary.web.home.stats.description;
  const items = cmsData?.items?.items ?? dictionary.web.home.stats.items;
