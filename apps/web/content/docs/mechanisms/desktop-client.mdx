...
- One in the private build repository, carrying the updater metadata (`latest-mac.yml`) and the mac zip assets that the update service reads on the client's behalf.
- One in the public `closedloop-ai/closedloop-ai` repository, carrying a single fixed-name universal DMG — the stable, account-free URL behind the public download link.
