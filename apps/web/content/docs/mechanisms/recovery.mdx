...
- Preserves a matching `plan.json` when one is already present and consistent with the run, so recovery does not overwrite a successfully synthesized plan with a stale snapshot.
