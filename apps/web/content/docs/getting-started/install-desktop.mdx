...
- **macOS:** download the latest signed, notarized universal DMG from the Desktop release channel under `closedloop-ai/symphony-alpha`. Drag the `Closedloop.app` bundle into `/Applications`.
...
Packaged macOS builds auto-update via the Desktop-specific `symphony-alpha` feed. The client checks on boot and every five minutes after that, downloading updates in the background and installing them on the next quit.

Older builds installed from `closedloop-ai/closedloop-electron` do not auto-upgrade across repositories. Install the latest `closedloop-ai/symphony-alpha` Desktop DMG manually once; subsequent updates follow the new Desktop feed.
...
|---|---|
| `just desktop-build` | Build the Electron app. |
| `just desktop-stage` | Materialize the packaged production dependency closure. |
