...
> **Branch:** `runtimev2` (closedloop-ai) + `runtimev2` (cl-tofu-aws-live)
...
| 0.3 | **App code PR ready** — open PR from `runtimev2` → `main` in `closedloop-ai`, mark as draft | PR exists, reviewed, lint/typecheck/tests green | Low |
...
| # | Step | How | Depends on | Estimated time |
|---|------|-----|------------|----------------|
| 3.1 | **Merge app PR to main** | Merge `runtimev2` → `main` in closedloop-ai | Phase 0.3 (PR reviewed) | Immediate |
