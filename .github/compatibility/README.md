...
- **`symphonyAlpha.sha`** — The commit SHA of this repository (`closedloop-ai`) that was part of the last verified compatible pair.
...
The auto-update workflow lives in the `closedloop-electron` repository. After a successful compatibility test run, it pushes an updated version of this file to `closedloop-ai` via a Personal Access Token (PAT). This keeps the file current without requiring manual intervention after each passing test.
